...
- The chart can now be scaled up in `standalone` mode. Proxy replication requires
  a TLS certificate; Auth replication requires using [HA storage backends](/reference/backends).
...
[Kubernetes 1.25 PSP removal guide](/deploy-a-cluster/helm-deployments/migration-kubernetes-1-25-psp).
...
- [backup the cluster content](/management/operations/backup-restore),
