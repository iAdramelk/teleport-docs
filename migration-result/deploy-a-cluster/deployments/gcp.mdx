...
see  [Admin Guide: Troubleshooting](/management/admin/troubleshooting) for more information.
...
Follow install instructions from our [installation page](/installation#linux).
...
<Tabs>
  <Tab title="Open Source">
    **1. Configure Teleport Auth Server** using the below example `teleport.yaml`,and start it
    using [systemd](/management/admin/daemon). The DEB/RPM installations will
...
Follow our [Local Users](/management/admin/users) guide or integrate
with [Google Workspace](/access-controls/sso/google-workspace) to
provide SSO access.
