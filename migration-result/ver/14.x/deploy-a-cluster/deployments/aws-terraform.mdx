...
More information about how Teleport works with DynamoDB can be found in our [Storage Backends guide](/ver/14.x/reference/backends#dynamodb).
...
ways to integrate Teleport onto your servers. We recommend looking at our [Installation guide](/ver/14.x/installation).

To add new nodes/EC2 servers that you can "SSH into" you'll need to:

- [Install the Teleport binary on the Server](/ver/14.x/installation)
- [Run Teleport - we recommend using systemd](/ver/14.x/management/admin/daemon)
- [Set the correct settings in /etc/teleport.yaml](/ver/14.x/reference/config)
- [Add Nodes to the Teleport
  cluster](/ver/14.x/agents/join-services-to-your-cluster)
...
You can also generate a Node join token using `tctl tokens add --type=node` [as detailed here in our admin guide](/ver/14.x/agents/join-services-to-your-cluster/join-token).
...
for this document) or join your nodes using [Teleport's node tunneling](/ver/14.x/agents/join-services-to-your-cluster/join-token) functionality.
...
to an auth server. Follow the instructions in our [Trusted Clusters guide](/ver/14.x/management/admin/trustedclusters).
