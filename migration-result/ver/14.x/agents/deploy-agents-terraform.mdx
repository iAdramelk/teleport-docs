...
Agents](/ver/14.x/agents/introduction).
...
Cluster](/ver/14.x/agents/join-services-to-your-cluster) guide. In this guide, we will use
...
<Tip>
  ...
  - A demo deployment on a [Linux server](/ver/14.x/index)
...
- An identity file for the Teleport Terraform provider. Make sure you are
  ...
  provider](/ver/14.x/management/dynamic-resources/terraform-provider) before following this
  guide.
...
```code
$ sed -i "" "s/TELEPORT_VERSION/12.1.1/" provider.tf
```
...
reference](/ver/14.x/reference/terraform-provider).
