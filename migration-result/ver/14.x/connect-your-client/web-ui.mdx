...
you `list` access for the `ssh_session` resource. Please refer to the [Teleport Access Controls Reference](/ver/14.x/access-controls/reference)
...
If the launch button is missing, then you don't have permission to join in any participant mode. Please refer to the [`join_sessions` allow policy reference](/ver/14.x/access-controls/guides/moderated-sessions#join_sessions) to learn how to grant roles access to participant modes.
