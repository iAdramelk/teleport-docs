...
[`tsh`](/ver/14.x/reference/cli#tsh):
...
to call [`tsh login`](/ver/14.x/reference/cli#tsh-login) in the beginning.
...
<Tabs>
  ...
  <Tab title="Mac - Homebrew">
    ...
    <Note>
      ...
      versions. See our [compatibility policy](/ver/14.x/management/operations/upgrading#component-compatibility) for details.
    </Note>
  </Tab>
  ...
  <Tab title="Linux">
    ...
    For more options (including RPM/DEB packages and downloads for i386/ARM/ARM64) please see our [installation page](/ver/14.x/installation).
...
[CLI Docs - tsh ssh](/ver/14.x/reference/cli#tsh-ssh)
...
[CLI Docs - tsh login](/ver/14.x/reference/cli#tsh-login)
...
<Tip>
  It is recommended to always use [`tsh login`](/ver/14.x/reference/cli#tsh-login) before using any other `tsh` commands. This allows users to omit `--proxy` flag in subsequent tsh commands. For example `tsh ssh user@host` will work.
</Tip>

A Teleport cluster can be configured for multiple user identity sources. For example, a cluster may have a local user called `admin` while regular users should [authenticate via GitHub](/ver/14.x/access-controls/sso/github-sso). In this case, you have to pass `--auth` flag to `tsh login` to specify which identity storage to use:
...
[CLI Docs - tsh status](/ver/14.x/reference/cli#tsh-status)
...
[`tsh login`](/ver/14.x/reference/cli#tsh-login) can also save the user certificate into a
file:
...
[Machine ID](/ver/14.x/machine-id/introduction). This ensures that your automation
...
[CLI Docs - tctl auth sign](/ver/14.x/reference/cli#tctl-auth-sign)
...
[CLI Docs - tsh ls](/ver/14.x/reference/cli#tsh-ls)
...
<Tip>
  ...
  Refer them to the [Moderated Sessions guide](/ver/14.x/access-controls/guides/moderated-sessions) for more information on configuring join permissions.
</Tip>
...
These features are called **Trusted Clusters**. Refer to [the Trusted Clusters guide](/ver/14.x/management/admin/trustedclusters)
...
[CLI Docs - tsh clusters](/ver/14.x/reference/cli#tsh-clusters)
...
[Uninstalling Teleport](/ver/14.x/management/admin/uninstall-teleport).

## Further reading

- [CLI Reference](/ver/14.x/reference/cli).
