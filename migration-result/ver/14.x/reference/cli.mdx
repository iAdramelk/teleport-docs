...
- [Auth](/ver/14.x/architecture/authentication)
- [Node/SSH](/ver/14.x/architecture/nodes)
- [Proxy](/ver/14.x/architecture/proxy)
- [App](/ver/14.x/application-access/introduction)
- [Database](/ver/14.x/database-access/introduction)
- [Windows Desktop](/ver/14.x/desktop-access/introduction)
- [Kubernetes](/ver/14.x/kubernetes-access/introduction)
...
| `teleport start`                       | Starts the `teleport` process in the foreground using the current shell session, including any services configured by the [configuration YAML file](/ver/14.x/reference/config).                                                                    |
| `teleport status`                      | Prints the status of the current active Teleport SSH session.                                                                                                                                                                                       |
| `teleport configure`                   | Generates and writes a [configuration YAML file](/ver/14.x/reference/config) for the Teleport service. This file should be customized in production to suit the needs of your environment, and the default output should only be used when testing. |
...
we recommend using a [configuration file](/ver/14.x/reference/config) in production.
...
| `-r, --roles`          | `proxy`, `node`, `auth`                                 | **string** comma-separated list of `proxy`, `node`, `auth`, `db`, or `app` | start listed services/roles. These roles are explained in the [Core Concepts](/ver/14.x/core-concepts) document.                                                                                                                                              |
...
| `--labels`             | none                                                    | **string** comma-separated list                                            | assigns a set of labels to a node, for example env=dev,app=web. See the explanation of labeling mechanism in the [Labeling Nodes](/ver/14.x/management/admin/labels) section.                                                                                 |
...
| ------------------------------------------------ | --------- | -------------------------------------------------------------------------------------------------------------------------- |
| [Node](/ver/14.x/architecture/nodes)             | `node`    | Allows SSH connections from authenticated clients.                                                                         |
| [Auth](/ver/14.x/architecture/authentication)    | `auth`    | Authenticates and authorizes hosts and users who want access to Teleport-managed resources or information about a cluster. |
| [Proxy](/ver/14.x/architecture/proxy)            | `proxy`   | The gateway that clients use to connect to the Auth Service or resources managed by Teleport.                              |
| [App](/ver/14.x/application-access/introduction) | `app`     | Provides access to applications.                                                                                           |
| [Database](/ver/14.x/database-access/reference)  | `db`      | Provides access to databases.                                                                                              |
...
| `--auth`               | `local`          | Any defined [authentication connector](/ver/14.x/reference/authentication), including `passwordless` and `local` (i.e., no authentication connector) | Specify the type of authentication connector to use.                                                                                                                                      |
...
<Tip>
  ...
  in their [storage backend](/ver/14.x/reference/backends).
...
through Teleport.  See the [OpenSSH Guide](/ver/14.x/server-access/guides/openssh) guide
...
Access](/ver/14.x/application-access/cloud-apis/azure).
...
- `<cluster>` - the name of the cluster,  see [Trusted Cluster](/ver/14.x/management/admin/trustedclusters) for more information.
...
Log into a Kubernetes cluster. Discover connected clusters by using [`tsh kube ls`](/ver/14.x/reference/cli#tsh-kube-ls).
...
registered devices using [`tsh mfa ls`](/ver/14.x/reference/cli#tsh-mfa-ls).
...
| `--query`  | none             | String                                                       | Query by [predicate language](/ver/14.x/reference/predicate-language#resource-filtering) enclosed in single quotes |
...
<Tabs>
  <Tab title="Self-Hosted">
    ...
    <Note>
      ...
      [Access Controls](/ver/14.x/access-controls/introduction) and ensure that
...
<Tip>
  ...
  [Machine ID](/ver/14.x/machine-id/introduction) is a more secure alternative that
...
| `--google-legacy`  | Flag to select groups with direct membership filtered by domain (legacy behavior). <br />Disabled by default. [More info](/ver/14.x/access-controls/sso/google-workspace#how-teleport-uses-google-workspace-apis) |
...
See the [Resources Reference](/ver/14.x/reference/resources) for complete docs on how to build these yaml files.
...
The `--query` flag allows you to perform more sophisticated searches using a [predicate language](/ver/14.x/reference/predicate-language#resource-filtering).
...
Refer to the [database guide](/ver/14.x/machine-id/guides/databases) for more information on
