...
This includes deploying proxies, auth servers, and [kubernetes-access](/ver/14.x/kubernetes-access/introduction).
See the [Teleport HA Architecture page](/ver/14.x/deploy-a-cluster/high-availability)
for more details.
...
| -------------------- | ------------------ | ---------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| `auth_service`       | auth `Deployment`  | Authenticates users and hosts, and issues certificates.                                                                | [Auth documentation](/ver/14.x/architecture/authentication)                 |
| `kubernetes_service` | auth `Deployment`  | Provides secure access to the Kubernetes<br />cluster where the Teleport cluster is hosted.                            | [Kubernetes Access documentation](/ver/14.x/kubernetes-access/introduction) |
| `proxy_service`      | proxy `Deployment` | Runs the externally-facing parts of a Teleport<br />cluster, such as the web UI, SSH proxy and reverse tunnel service. | [Proxy documentation](/ver/14.x/architecture/proxy)                         |
...
<Tip>
  ...
  [`teleport-kube-agent` Helm chart](/ver/14.x/reference/helm-reference/teleport-kube-agent).
...
| ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| `standalone`              | Runs by relying only on Kubernetes resources.                                                                                                                                                                                     | [Getting Started - Kubernetes with SSO](/ver/14.x/deploy-a-cluster/helm-deployments/kubernetes-cluster)            |
| `aws`                     | Leverages AWS managed services to store data.                                                                                                                                                                                     | [Running an HA Teleport cluster using an AWS EKS Cluster](/ver/14.x/deploy-a-cluster/helm-deployments/aws)         |
| `gcp`                     | Leverages GCP managed services to store data.                                                                                                                                                                                     | [Running an HA Teleport cluster using a Google Cloud GKE cluster](/ver/14.x/deploy-a-cluster/helm-deployments/gcp) |
| `scratch` (v12 and above) | Generates empty Teleport configuration. User must pass their own config. This is discouraged, use `standalone` mode with [`auth.teleportConfig`](#authteleportconfig) and [`proxy.teleportConfig`](#proxyteleportconfig) instead. | [Running a Teleport cluster with a custom config](/ver/14.x/deploy-a-cluster/helm-deployments/custom)              |

<Note>
  `custom` mode has been removed in Teleport version 12. See the [version 12
  migration guide](/ver/14.x/deploy-a-cluster/helm-deployments/migration-v12) for
  more information.
</Note>
...
See the [Teleport Configuration Reference](/ver/14.x/reference/config) for the list of supported fields.
...
[The SSO documentation](/ver/14.x/access-controls/sso) explains how to create
...
- [`local`](/ver/14.x/management/admin/users) for local users
- [`passwordless`](/ver/14.x/access-controls/guides/passwordless#optional-enable-passwordless-by-default) to enable by
...
[Disabling local auth is required for FedRAMP / FIPS](/ver/14.x/access-controls/compliance-frameworks/fedramp#teleport-auth-server).
...
See [the locking modes documentation](/ver/14.x/access-controls/guides/locking#next-steps-locking-modes) for more
details.
...
See [Second Factor - WebAuthn](/ver/14.x/access-controls/guides/webauthn) for more details.
...
For possible values, [see the Teleport Configuration Reference](/ver/14.x/reference/config).
...
See our [installation guide](/ver/14.x/installation#docker) for information on
...
[PSP removal guide](/ver/14.x/deploy-a-cluster/helm-deployments/migration-kubernetes-1-25-psp).
...
| `chartMode`  | Guide                                                                                                              |
| ------------ | ------------------------------------------------------------------------------------------------------------------ |
| `standalone` | [Getting Started - Kubernetes with SSO](/ver/14.x/deploy-a-cluster/helm-deployments/kubernetes-cluster)            |
| `aws`        | [Running an HA Teleport cluster using an AWS EKS Cluster](/ver/14.x/deploy-a-cluster/helm-deployments/aws)         |
| `gcp`        | [Running an HA Teleport cluster using a Google Cloud GKE cluster](/ver/14.x/deploy-a-cluster/helm-deployments/gcp) |
| `scratch`    | [Running a Teleport cluster with a custom config](/ver/14.x/deploy-a-cluster/helm-deployments/custom)              |
...
`aws` settings are described in the AWS guide: [Running an HA Teleport cluster using an AWS EKS Cluster](/ver/14.x/deploy-a-cluster/helm-deployments/aws)

## `gcp`

`gcp` settings are described in the GCP guide: [Running an HA Teleport cluster using a Google Cloud GKE cluster](/ver/14.x/deploy-a-cluster/helm-deployments/gcp)
...
<Note>
  ...
  and the relevant sections of the [AWS](/ver/14.x/deploy-a-cluster/helm-deployments/aws#step-36-configure-tls-certificates-for-teleport) and [GCP](/ver/14.x/deploy-a-cluster/helm-deployments/gcp#step-36-install-and-configure-cert-manager) guides for more information.
</Note>
...
<Note>
  ...
  of the [AWS](/ver/14.x/deploy-a-cluster/helm-deployments/aws#step-36-configure-tls-certificates-for-teleport) and [GCP](/ver/14.x/deploy-a-cluster/helm-deployments/gcp#step-36-install-and-configure-cert-manager) guides for more information.
</Note>
...
See the [Teleport config file reference](/ver/14.x/reference/config) for more details on possible values for `extra_fields`.
