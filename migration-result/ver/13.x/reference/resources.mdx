...
Resources](/ver/13.x/management/dynamic-resources).
...
Here's the list of resources currently exposed via [`tctl`](/ver/13.x/reference/cli#tctl):
...
| connector                 | Authentication connectors for [Single Sign-On](/ver/13.x/access-controls/sso) (SSO) for SAML, OIDC and GitHub.              |
| node                      | A registered SSH node. The same record is displayed via `tctl nodes ls`                                                     |
| cluster                   | A trusted cluster. See [here](/ver/13.x/management/admin/trustedclusters) for more details on connecting clusters together. |
| login\_rule               | A Login Rule, see the [Login Rules guide](/ver/13.x/access-controls/login-rules) for more info.                             |
| device                    | A Teleport Trusted Device, see the [Device Trust guide](/ver/13.x/access-controls/device-trust/guide) for more info.        |
