...
Agents](/ver/13.x/agents/introduction).
...
Cluster](/ver/13.x/agents/join-services-to-your-cluster) guide. In this guide, we will use
...
<Tip>
  ...
  - A demo deployment on a [Linux server](/ver/13.x/index)
...
- An identity file for the Teleport Terraform provider. Make sure you are
  ...
  provider](/ver/13.x/management/dynamic-resources/terraform-provider) before following this
  guide.
...
```code
$ sed -i "" "s/TELEPORT_VERSION/13.2.0/" provider.tf
```
...
reference](/ver/13.x/reference/terraform-provider).
