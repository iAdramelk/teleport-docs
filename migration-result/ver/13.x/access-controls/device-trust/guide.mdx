...
- To enroll a macOS device, you need:
  - A signed and notarized `tsh` binary for enrollment.
    [Download the macOS tsh installer](/ver/13.x/installation#macos).
  - A Teleport version newer than v12.0.0.
- To enroll a Windows device, you need:
  - A device that includes a TPM with 2.0 support.
  - `tsh` installed on the device. [Download the Windows tsh installer](/ver/13.x/installation#windows-tsh-client-only).
...
<Tip>
  ...
  clusters](/ver/13.x/management/admin/trustedclusters) to limit the impact of
...
works similar to [`require_session_mfa`](/ver/13.x/access-controls/guides/per-session-mfa).
...
Similar to [session and identity locking](/ver/13.x/access-controls/guides/locking), a device can
...
Refer to the [auto-enrollment guide](/ver/13.x/access-controls/device-trust/auto-enrollment).
...
device. [Download the macOS tsh installer](/ver/13.x/installation#macos) to fix
the problem.
...
[macOS tsh installer](/ver/13.x/installation#macos).

A trusted device needs to be registered and enrolled before it is recognized by
Teleport as such. Follow the [registration](/ver/13.x/access-controls/device-trust/guide#step-12-register-a-trusted-device) and
[enrollment](/ver/13.x/access-controls/device-trust/guide#step-22-enroll-a-trusted-device) steps
