...
- The chart can now be scaled up in `standalone` mode. Proxy replication requires
  a TLS certificate; Auth replication requires using [HA storage backends](/ver/16.x/reference/backends).
...
[Kubernetes 1.25 PSP removal guide](/ver/16.x/deploy-a-cluster/helm-deployments/migration-kubernetes-1-25-psp).
...
- [backup the cluster content](/ver/16.x/management/operations/backup-restore),
