...
If set to the string `true`, Teleport will use [TLS routing](/ver/16.x/architecture/tls-routing) to multiplex all traffic over a single port.
...
See the [Teleport authentication reference](/ver/16.x/reference/authentication) for more information.
...
More information about how Teleport works with DynamoDB can be found in our [Storage Backends guide](/ver/16.x/reference/backends#dynamodb).
...
ways to integrate Teleport onto your servers. We recommend looking at our [Installation guide](/ver/16.x/installation).

To add new nodes/EC2 servers that you can "SSH into" you'll need to:

- [Install the Teleport binary on the Server](/ver/16.x/installation)
- [Run Teleport - we recommend using systemd](/ver/16.x/management/admin/daemon)
- [Set the correct settings in /etc/teleport.yaml](/ver/16.x/reference/config)
- [Add Nodes to the Teleport
  cluster](/ver/16.x/agents/join-services-to-your-cluster)
...
You can also generate an SSH Service join token using `tctl tokens add --type=node` [as detailed here in our admin guide](/ver/16.x/agents/join-services-to-your-cluster/join-token).
...
To manually join Teleport agents to your cluster, you will need [a join token](/ver/16.x/agents/join-services-to-your-cluster/join-token).
...
to a Teleport Auth Service instance. Follow the instructions in our [Trusted Clusters guide](/ver/16.x/management/admin/trustedclusters).
