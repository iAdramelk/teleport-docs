...
- Teleport Connect v14.1+, on the same major version or one version behind the Teleport Proxy
  Service version. See [version compatibility](/ver/16.x/faq#version-compatibility).
- A local Teleport user: you must authenticate using credentials or passwordless login, and not with
  SSO.
- Permissions to create join tokens (verb `create` for [the `token` resource](/ver/16.x/access-controls/reference#rbac-for-dynamic-teleport-resources)).
...
- Permissions to read and update user objects in the backend (verbs `read` and `update` for [the
  `user` resource](/ver/16.x/access-controls/reference#rbac-for-dynamic-teleport-resources)).
- Permissions to read, update, and create roles in the backend (verbs `read`, `update`, and `create`
  for [the `role` resource](/ver/16.x/access-controls/reference#rbac-for-dynamic-teleport-resources)).

The agent runs as the current system user, not as root. Some features are thus not available, such
as logging in as other system users or [host user creation](/ver/16.x/server-access/guides/host-user-creation).
...
During the setup, Connect My Computer creates [a join token](/ver/16.x/agents/join-services-to-your-cluster/join-token)
...
Controls Reference Documentation](/ver/16.x/access-controls/reference#rbac-for-dynamic-teleport-resources) for more
...
locks](/ver/16.x/access-controls/guides/locking) on those nodes.
...
When using [trusted clusters](/ver/16.x/management/admin/trustedclusters), the cluster selector allows
...
variable](/ver/16.x/reference/cli/tsh#tsh-environment-variables). For example, logging in to a new cluster
...
certificates](/ver/16.x/management/admin/self-signed-certs) or for demo purposes. We do not recommend
