...
- If you have not already put your database behind the Teleport Database Service,
  follow the [database access getting started guide](/ver/16.x/database-access/getting-started).
  The Teleport Database Service supports databases like PostgreSQL, MongoDB,
  Redis, and much more. See our [database access
  guides](/ver/16.x/database-access/guides) for a complete list.
...
- The `tsh` binary must be installed on the machine that will access the
  ...
  installed. If it is not, see [Installation](/ver/16.x/installation) for
  details.
...
[Database Access RBAC guide](/ver/16.x/database-access/rbac/configuring-access)
