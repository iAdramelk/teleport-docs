...
As you learned in [Core Concepts](/ver/16.x/core-concepts), a Teleport cluster
...
through an external identity provider, see [Configure Single Sign-on](/ver/16.x/access-controls/sso).
...
- The `tctl` admin tool and `tsh` client tool version >= 15.0.0-dev.
  ...
  For more information about installing Teleport, see [Installation](/ver/16.x/installation).
...
- A Teleport SSH server that is joined to the cluster you plan to use as the **leaf cluster**.
  ...
  [Join Services to your Cluster](/ver/16.x/agents/join-services-to-your-cluster).
