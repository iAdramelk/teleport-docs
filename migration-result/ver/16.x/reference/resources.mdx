...
Resources](/ver/16.x/management/dynamic-resources).
...
Here's the list of resources currently exposed via [`tctl`](/ver/16.x/reference/cli/tctl):
...
| connector                                              | Authentication connectors for [Single Sign-On](/ver/16.x/access-controls/sso) (SSO) for SAML, OIDC and GitHub.              |
| node                                                   | A registered SSH node. The same record is displayed via `tctl nodes ls`                                                     |
| windows\_desktop                                       | A registered Windows desktop.                                                                                               |
| cluster                                                | A trusted cluster. See [here](/ver/16.x/management/admin/trustedclusters) for more details on connecting clusters together. |
| [login\_rule](#login-rules)                            | A Login Rule, see the [Login Rules guide](/ver/16.x/access-controls/login-rules) for more info.                             |
| [device](#device)                                      | A Teleport Trusted Device, see the [Device Trust guide](/ver/16.x/access-controls/device-trust/guide) for more info.        |
...
[Machine ID configuration reference](/ver/16.x/machine-id/reference/configuration).
