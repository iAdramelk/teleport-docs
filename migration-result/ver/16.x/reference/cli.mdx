...
- [teleport](/ver/16.x/reference/cli/teleport): Supports the Teleport Access Platform by starting and configuring various Teleport services.
- [tsh](/ver/16.x/reference/cli/tsh): Allows end users to authenticate to Teleport and access resources in a cluster.
- [tctl](/ver/16.x/reference/cli/tctl): Used to configure the Teleport Auth Service.
- [tbot](/ver/16.x/reference/cli/tbot): Supports Machine ID, which provides short lived credentials to service accounts (e.g, a CI/CD server).
...
The `--query` flag allows you to perform more sophisticated searches using a [predicate language](/ver/16.x/reference/predicate-language#resource-filtering).
