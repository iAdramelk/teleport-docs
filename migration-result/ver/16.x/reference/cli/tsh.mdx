...
| `--auth`               | `local`          | Any defined [authentication connector](/ver/16.x/reference/authentication), including `passwordless` and `local` (i.e., no authentication connector) | Specify the type of authentication connector to use.                                                                                                                                      |
...
- `<cluster>` - the name of the cluster,  see [Trusted Cluster](/ver/16.x/management/admin/trustedclusters) for more information.
...
Access](/ver/16.x/application-access/cloud-apis/azure).
...
through Teleport.  See the [OpenSSH Guide](/ver/16.x/server-access/guides/openssh) guide
...
See [full docs on `tsh puttyconfig` here](/ver/16.x/connect-your-client/putty-winscp).
...
<Tip>
  ...
  in their [storage backend](/ver/16.x/reference/backends).
...
| `--query`  | none             | String                                                       | Query by [predicate language](/ver/16.x/reference/predicate-language#resource-filtering) enclosed in single quotes |
