...
reference](/reference/cli/tctl) for more information on `tctl`.
...
[Get started with the Terraform provider](/management/dynamic-resources/terraform-provider).
...
[Get started with the Kubernetes Operator](/management/dynamic-resources/teleport-operator).
...
- To see the dynamic configuration resources available to apply, read the
  [Configuration Resource Reference](/reference/resources). There are also
  dedicated configuration resource references for
  [applications](/application-access/reference#application-resource) and
  [databases](/database-access/reference/configuration#database-resource).
