...
SSH sessions are recorded [on nodes](/architecture/nodes).
...
The Teleport [Auth Service](/architecture/authentication) is deployed within the AWS us-west-2 region in 4 availability zones, and can tolerate a single zone failure. AWS guarantees [99.99%](https://aws.amazon.com/compute/sla/) of monthly uptime.

### Proxies

The Teleport [Proxy Service](/architecture/proxy) is deployed to multiple AWS regions around the world for low-latency access to distributed infrastructure.
