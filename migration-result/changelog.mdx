...
See the documentation [here](/reference/backends#athena).
...
You can find existing access lists documentation [here](/access-controls/access-lists/guide).
...
See documentation [here](/application-access/enroll-kubernetes-applications).
...
resources](/kubernetes-access/controls#kubernetes_resources).
...
See how to connect ClickHouse to Teleport [here](/database-access/guides/clickhouse-self-hosted).
...
See documentation on how to configure it in the [Oracle guide](/database-access/guides/oracle-self-hosted).
...
For more information on using Teleport with local Windows users, see [docs](/desktop-access/getting-started).
...
For more information, see [docs](/connect-your-client/putty-winscp).
...
For more information, see [docs](/machine-id/reference/configuration#kubernetes_secret).
...
the cluster. Follow the updated agentless OpenSSH integration [guide](/server-access/guides/openssh)
...
See more details in our [documentation](/management/security/proxy-protocol).
...
using [header rewriting](/application-access/jwt/introduction#inject-jwt).
...
You can review role versions in the [documentation](/access-controls/reference#role-versions).
...
For more details and guidance on how to upgrade to v2, see [docs](/machine-id/reference/v14-upgrade-guide).
...
guide](/server-access/guides/openssh).
...
guide](/deploy-a-cluster/helm-deployments/migration-v12).
...
follow
the [migration guide](/deploy-a-cluster/helm-deployments/migration-v12).
...
[documentation](/machine-id/deployment/github-actions) for using
...
[documentation](/access-controls/guides/hardware-key-support):
...
- [VS Code](/server-access/guides/vscode)
- [JetBrains](/server-access/guides/jetbrains-sftp)
...
guide](/database-access/guides/azure-postgres-mysql) for more
details.
...
[install Teleport](/installation#linux).
...
See the [documentation](/access-controls/guides/passwordless).
...
[documentation](/server-access/guides/host-user-creation).
...
Refer to [the guide](/database-access/guides/sql-server-ad) to set
...
[documentation](/database-access/guides/snowflake).
...
Learn more about it in the [documentation](/database-access/guides/redis-aws).
...
[documentation](/machine-id/access-guides/databases).
...
[documentation](/management/operations/upgrading).
...
Refer to the
[documentation](/reference/networking#http-connect-proxies)
for more details.
...
instructions](/server-access/getting-started#step-14-install-teleport-on-your-linux-host).
...
guide](/access-controls/guides/moderated-sessions) for more
details.
...
documentation](/reference/backends) for more information.
...
configuration](/desktop-access/getting-started) to force Windows
...
See the [documentation](/reference/backends) for more information.
...
[Machine ID getting started guide](/machine-id/getting-started)
...
[Self-hosted Redis guide](/database-access/guides/redis)
...
[SQL Server guide](/database-access/guides/sql-server-ad)
...
[Updated RDS guide](/database-access/guides/rds)
...
- [`teleport db configure
  create`](/database-access/reference/cli#teleport-db-configure-create)
- [`teleport db configure bootstrap`](/database-access/reference/cli#teleport-db-configure-bootstrap)
...
[Moderated Sessions guide](/access-controls/guides/moderated-sessions)
...
[Getting Started Guide](/desktop-access/getting-started).
...
[documentation](/application-access/cloud-apis/aws-console).
...
[apps](/application-access/guides/dynamic-registration)
and
[databases](/database-access/guides/dynamic-registration).
...
See updated
[RDS guide](/database-access/guides/rds) for
more information.
...
View the [database access with MongoDB](/database-access/guides/mongodb-self-hosted) for more details.
...
View the Cloud SQL MySQL [guide](/database-access/guides/mysql-cloudsql) for more details.
...
Teleport application access can now automatically sign users into the AWS Management Console using [Identity federation](https://aws.amazon.com/identity/federation). View AWS Management Console [guide](/application-access/cloud-apis/aws-console) for more details.
...
View the [database access with Redshift on AWS guide](/database-access/guides/postgres-redshift) for more details.
...
MFA](/access-controls/guides/per-session-mfa) documentation or
[RFD
...
We have implemented [database access](/database-access/introduction),
...
Configure database access following the [Getting Started](/database-access/introduction#getting-started) guide.

##### Guides

- [AWS RDS/Aurora PostgreSQL](/database-access/guides/rds)
- [AWS RDS/Aurora MySQL](/database-access/guides/rds)
- [Self-hosted PostgreSQL](/database-access/guides/postgres-self-hosted)
- [Self-hosted MySQL](/database-access/guides/mysql-self-hosted)
- [GUI clients](/connect-your-client/gui-clients)

##### Resources

To learn more about configuring role-based access control for database access, check out the [RBAC](/database-access/introduction) section.

[Architecture](/database-access/introduction) provides a more in-depth look at database access internals such as networking and security.

See [Reference](/database-access/reference) for an overview of database access related configuration and CLI commands.
...
Please follow our [standard upgrade procedure](/management/admin) to upgrade your cluster.
...
You can learn more in the [Application Access introduction](/application-access/introduction).
...
Learn more about [Teleport's RBAC Resources](/access-controls/introduction)

##### Cluster Labels

Teleport 5.0 also adds the ability to set labels on Trusted Clusters. The labels are set when creating a trusted cluster invite token. This lets teams use the same RBAC controls used on nodes to approve or deny access to clusters. This can be especially useful for MSPs that connect hundreds of customers' clusters - when combined with Access Workflows, cluster access can easily be delegated. Learn more by reviewing our [Truster Cluster Setup & RBAC Docs](/management/admin/trustedclusters)
...
- We've added a Waiting Room for customers using Access Workflows. [Docs](/access-controls/access-request-plugins/index)
...
We've added an [API Guide](/api/introduction) to simply developing applications against Teleport.

#### Upgrade Notes

Please follow our [standard upgrade
procedure](/management/admin/upgrading-the-teleport-binary).
...
Please follow our [standard upgrade
procedure](/management/operations/upgrading).
...
Teleport 4.3 introduces four new plugins that work out of the box with [Approval Workflow](/access-controls/access-request-plugins/index). These plugins allow you to automatically support role escalation with commonly used third party services. The built-in plugins are listed below.

- [PagerDuty](/access-controls/access-request-plugins/ssh-approval-pagerduty)
- [Jira](/access-controls/access-request-plugins/ssh-approval-jira)
- [Slack](/access-controls/access-request-plugins/ssh-approval-slack)
- [Mattermost](/access-controls/access-request-plugins/ssh-approval-mattermost)
...
procedure](/management/operations/upgrading) to upgrade to this version.
...
docs](/management/operations/ca-rotation).
...
permissions](/kubernetes-access/manage-access/rbac).

##### Path prefix for etcd

The [etcd backend](/reference/backends#etcd) now correctly uses
...
[upgrade guidance](/management/operations/upgrading).

**Note: If you use an etcd backend with a non-default prefix and need to downgrade from 4.3 to 4.2, you should [backup Teleport data and restore it](/management/operations/backup-restore) into the downgraded cluster.**
...
- Alpha: Workflows API lets admins escalate RBAC roles in response to user requests. [Read the docs](/access-controls/access-requests). [#3006](https://github.com/gravitational/teleport/issues/3006)
- Beta: Teleport provides HA Support using Firestore and Google Cloud Storage using Google Cloud Platform. [Read the docs](/deploy-a-cluster/deployments/gcp). [#2821](https://github.com/gravitational/teleport/pull/2821)
- Remote tctl execution is now possible. [Read the docs](/reference/cli/tctl). [#1525](https://github.com/gravitational/teleport/issues/1525) [#2991](https://github.com/gravitational/teleport/issues/2991)
...
- Adopting root/leaf terminology for trusted clusters. [Trusted cluster documentation](/management/admin/trustedclusters).
- Documented Teleport FedRAMP & FIPS Support. [FedRAMP & FIPS documentation](/access-controls/compliance-frameworks/fedramp).
...
- Teleport now support 10,000 remote connections to a single Teleport cluster. [Using our recommend hardware setup.](/management/operations/scaling#hardware-recommendations)
...
Teleport 4.0 is backwards compatible with Teleport 3.2 and later. [Follow the recommended upgrade procedure to upgrade to this version.](/management/operations/upgrading)
...
procedure](/management/operations/upgrading) to upgrade to this
version.
...
- Role templates (depreciated in Teleport 2.3) were fully removed. We recommend
  migrating to role variables which are documented [here](/access-controls/guides/role-templates)
...
- The Teleport daemon now implements built-in connection draining which allows
  ...
  documentation](/management/admin/upgrading-the-teleport-binary).
