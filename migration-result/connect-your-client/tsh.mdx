...
to call [`tsh login`](/reference/cli/tsh#tsh-login) in the beginning.
...
<Tabs>
  ...
  <Tab title="Mac - Homebrew">
    ...
    <Note>
      ...
      versions. See our [compatibility policy](/management/operations/upgrading#component-compatibility) for details.
    </Note>
  </Tab>
  ...
  <Tab title="Linux">
    ...
    For more options (including RPM/DEB packages and downloads for i386/ARM/ARM64) please see our [installation page](/installation).
...
[CLI Docs - tsh ssh](/reference/cli/tsh#tsh-ssh)
...
[CLI Docs - tsh login](/reference/cli/tsh#tsh-login)
...
<Tip>
  It is recommended to always use [`tsh login`](/reference/cli/tsh#tsh-login) before using any other `tsh` commands. This allows users to omit `--proxy` flag in subsequent tsh commands. For example `tsh ssh user@host` will work.
</Tip>

A Teleport cluster can be configured for multiple user identity sources. For example, a cluster may have a local user called `admin` while regular users should [authenticate via GitHub](/access-controls/sso/github-sso). In this case, you have to pass `--auth` flag to `tsh login` to specify which identity storage to use:
...
[CLI Docs - tsh status](/reference/cli/tsh#tsh-status)
...
[`tsh login`](/reference/cli/tsh#tsh-login) can also save the user certificate into a
file:
...
[Machine ID](/machine-id/introduction). This ensures that your automation
...
[CLI Docs - tctl auth sign](/reference/cli/tctl#tctl-auth-sign)
...
[CLI Docs - tsh ls](/reference/cli/tsh#tsh-ls)
...
<Tip>
  ...
  Refer them to the [Moderated Sessions guide](/access-controls/guides/moderated-sessions) for more information on configuring join permissions.
</Tip>
...
[Configure Trusted Clusters](/management/admin/trustedclusters).
...
[CLI Docs - tsh clusters](/reference/cli/tsh#tsh-clusters)
...
[Uninstalling Teleport](/management/admin/uninstall-teleport).
