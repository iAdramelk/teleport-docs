...
- Teleport Connect v14.1+, on the same major version or one version behind the Teleport Proxy
  Service version. See [version compatibility](/faq#version-compatibility).
- A local Teleport user: you must authenticate using credentials or passwordless login, and not with
  SSO.
- Permissions to create join tokens (verb `create` for [the `token` resource](/access-controls/reference#rbac-for-dynamic-teleport-resources)).
...
- Permissions to read and update user objects in the backend (verbs `read` and `update` for [the
  `user` resource](/access-controls/reference#rbac-for-dynamic-teleport-resources)).

The agent runs as the current system user, not as root. Some features are thus not available, such
as logging in as other system users or [host user creation](/server-access/guides/host-user-creation).
...
During the setup, Connect My Computer creates [a join token](/agents/join-services-to-your-cluster/join-token)
...
Controls Reference Documentation](/access-controls/reference#rbac-for-dynamic-teleport-resources) for more
...
locks](/access-controls/guides/locking) on those nodes.
...
When using [trusted clusters](/management/admin/trustedclusters), the cluster selector allows
...
variable](/reference/cli/tsh#tsh-environment-variables). For example, logging in to a new cluster
...
certificates](/management/admin/self-signed-certs) or for demo purposes. We do not recommend
