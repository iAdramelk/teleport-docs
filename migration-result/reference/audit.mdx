...
<Tip>
  You can use
  [Enhanced Session Recording with BPF](/server-access/guides/bpf-session-recording)
...
<Tabs>
  <Tab title="Self-Hosted">
    ...
    [Athena](/reference/backends#athena), [DynamoDB](/reference/backends#dynamodb) or
    [Firestore](/reference/backends#firestore) chapters for information on how to
    ...
    [Teleport Configuration Reference](/reference/config).
...
<Tip>
  ...
  read [Exporting Teleport Audit Events](/management/export-audit-events).
</Tip>
...
<Tabs>
  <Tab title="Self-Hosted">
    Teleport can store the recorded sessions in an [AWS S3 bucket](/reference/backends#s3)
    ...
    You can replay recorded sessions using the [`tsh play`](/reference/cli/tsh#tsh-play)
