...
<Tip>
  Before continuing, please make sure to take a look at the [Cluster State section](/architecture/nodes#cluster-state)
...
address of the load balancer in the [`auth_server`](/reference/config) field when
...
If using the [default configuration](/reference/networking), configure your load
...
`/readyz` [diagnostics endpoint](/management/diagnostics/monitoring) on
...
<Note>
  ...
  [Teleport-event-exporter](/management/export-audit-events/fluentd)
