...
- [teleport](/reference/cli/teleport): Supports the Teleport Access Platform by starting and configuring various Teleport services.
- [tsh](/reference/cli/tsh): Allows end users to authenticate to Teleport and access resources in a cluster.
- [tctl](/reference/cli/tctl): Used to configure the Teleport Auth Service.
- [tbot](/reference/cli/tbot): Supports Machine ID, which provides short lived credentials to service accounts (e.g, a CI/CD server).
...
The `--query` flag allows you to perform more sophisticated searches using a [predicate language](/reference/predicate-language#resource-filtering).
