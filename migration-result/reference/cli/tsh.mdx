...
| `--auth`               | `local`          | Any defined [authentication connector](/reference/authentication), including `passwordless` and `local` (i.e., no authentication connector) | Specify the type of authentication connector to use.                                                                                                                                      |
...
See [full docs on `tsh puttyconfig` here](/connect-your-client/putty-winscp).
...
<Tip>
  ...
  in their [storage backend](/reference/backends).
...
through Teleport.  See the [OpenSSH Guide](/server-access/guides/openssh) guide
...
Access](/application-access/cloud-apis/azure).
...
- `<cluster>` - the name of the cluster,  see [Trusted Cluster](/management/admin/trustedclusters) for more information.
...
| `--query`  | none             | String                                                       | Query by [predicate language](/reference/predicate-language#resource-filtering) enclosed in single quotes |
