...
- [Node/SSH](/architecture/nodes)
- [Proxy](/architecture/proxy)
- [App](/application-access/introduction)
- [Database](/database-access/introduction)
- [Windows Desktop](/desktop-access/introduction)
- [Kubernetes](/kubernetes-access/introduction)
...
| `teleport start`                       | Starts the `teleport` process in the foreground using the current shell session, including any services configured by the [configuration YAML file](/reference/config).                                                                    |
| `teleport status`                      | Prints the status of the current active Teleport SSH session.                                                                                                                                                                              |
| `teleport configure`                   | Generates and writes a [configuration YAML file](/reference/config) for the Teleport service. This file should be customized in production to suit the needs of your environment, and the default output should only be used when testing. |
...
we recommend using a [configuration file](/reference/config) in production.
...
| `-r, --roles`          | `proxy`, `node`, `auth`                                 | **string** comma-separated list of `proxy`, `node`, `auth`, `db`, or `app` | start listed services/roles. These roles are explained in the [Core Concepts](/core-concepts) document.                                                                                                                                                       |
...
| `--labels`             | none                                                    | **string** comma-separated list                                            | assigns a set of labels to a node, for example env=dev,app=web. See the explanation of labeling mechanism in the [Labeling Nodes](/management/admin/labels) section.                                                                                          |
...
| --------------------------------------- | --------- | -------------------------------------------------------------------------------------------------------------------------- |
| [Node](/architecture/nodes)             | `node`    | Allows SSH connections from authenticated clients.                                                                         |
| [Auth](/architecture/authentication)    | `auth`    | Authenticates and authorizes hosts and users who want access to Teleport-managed resources or information about a cluster. |
| [Proxy](/architecture/proxy)            | `proxy`   | The gateway that clients use to connect to the Auth Service or resources managed by Teleport.                              |
| [App](/application-access/introduction) | `app`     | Provides access to applications.                                                                                           |
| [Database](/database-access/reference)  | `db`      | Provides access to databases.                                                                                              |
