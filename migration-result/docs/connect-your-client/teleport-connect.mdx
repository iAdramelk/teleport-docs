...
<Tabs>
  ...
  <Tab title="Linux">
    ...
    ```code
    $ tar -xf  teleport-connect-15.0.2-linux-*.tar.gz
    ```
  </Tab>
  ...
  <Tab title="Windows">
    ...
    ```code
    $ "Teleport Connect Setup-15.0.2.exe" /S
    ```
  </Tab>
</Tabs>
...
When using [trusted clusters](/docs/management/admin/trustedclusters), the cluster selector allows
...
When it comes to [cloud APIs secured with Application Access](/docs/application-access/cloud-apis),
...
- Teleport Connect v14.1+, on the same major version or one version behind the Teleport Proxy
  Service version. See [version compatibility](/docs/faq#version-compatibility).
- A local Teleport user: you must authenticate using credentials or passwordless login, and not with
  SSO.
- Permissions to create join tokens (verb `create` for [the `token` resource](/docs/access-controls/reference#rbac-for-dynamic-teleport-resources)).
...
- Permissions to read and update user objects in the backend (verbs `read` and `update` for [the
  `user` resource](/docs/access-controls/reference#rbac-for-dynamic-teleport-resources)).
- Permissions to read, update, and create roles in the backend (verbs `read`, `update`, and `create`
  for [the `role` resource](/docs/access-controls/reference#rbac-for-dynamic-teleport-resources)).

The agent runs as the current system user, not as root. Some features are thus not available, such
as logging in as other system users or [host user creation](/docs/server-access/guides/host-user-creation).
...
During the setup, Connect My Computer creates [a join token](/docs/agents/join-services-to-your-cluster/join-token)
...
Controls Reference Documentation](/docs/access-controls/reference#rbac-for-dynamic-teleport-resources) for more
...
locks](/docs/access-controls/guides/locking) on those nodes.
...
variable](/docs/reference/cli/tsh#tsh-environment-variables). For example, logging in to a new cluster
...
certificates](/docs/management/admin/self-signed-certs) or for demo purposes. We do not recommend
