...
or run a version server already, return to the [Upgrading](/docs/upgrading)
menu for the appropriate next steps to upgrade Teleport.

The [Automatic Update Architecture](/docs/architecture/agent-update-management)
...
3. Make the `current` release channel target the version 15.0.2:

   ```bash
   $ echo -n "15.0.2" > current/version
   ```
...
2. To suspend automatic updates for an agent, annotate the agent deployment
...
[`teleport_cluster_maintenance_config`](/docs/reference/terraform-provider#teleport_cluster_maintenance_config)
