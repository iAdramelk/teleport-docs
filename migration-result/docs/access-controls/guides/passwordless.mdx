...
- Teleport must be configured for WebAuthn. See the [Second Factor:
  WebAuthn](/docs/access-controls/guides/webauthn) guide.
...
- A signed and notarized version of `tsh` is required for Touch ID. This precludes
  versions installed from Homebrew. [Download the macOS tsh installer](/docs/installation#macos).
...
standalone version of `tsh`. [Download the macOS tsh installer](/docs/installation#macos).
