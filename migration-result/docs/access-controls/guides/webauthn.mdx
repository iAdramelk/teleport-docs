...
<Tip>
  ...
  *registered* security key" step. This may be undesirable at times, so you may
  ...
  Reference](/docs/reference/cli/tsh#tsh-global-flags) page.
</Tip>
...
<Note>
  WebAuthn for logging in to Teleport is only required for [local users](/docs/reference/authentication#local-no-authentication-connector).
...
- [Passwordless](/docs/access-controls/guides/passwordless)
- [Set up per-session MFA checks](/docs/access-controls/guides/per-session-mfa)
- [Require MFA for administrative actions](/docs/access-controls/guides/mfa-for-admin-actions)
