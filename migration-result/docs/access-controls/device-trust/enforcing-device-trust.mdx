...
works similarly to [`require_session_mfa`](/docs/access-controls/guides/per-session-mfa).
...
<Tip>
  ...
  clusters](/docs/management/admin/trustedclusters) to limit the impact of
...
be accessed using tsh commands such as [tsh proxy app](/docs/reference/cli/tsh#tsh-proxy-app) or using the certificates issued by
...
Similar to [session and identity locking](/docs/access-controls/guides/locking), a device can
...
device. [Download the macOS tsh installer](/docs/installation#macos) to fix
the problem.
...
Teleport as such. Follow the [registration](/docs/access-controls/device-trust/device-management#register-a-trusted-device) and
[enrollment](/docs/access-controls/device-trust/device-management#enroll-a-trusted-device) steps
...
[App Access support](/docs/access-controls/device-trust/enforcing-device-trust#app-access-support).
...
- [Device Management](/docs/access-controls/device-trust/device-management)
- [Jamf Pro Integration](/docs/access-controls/device-trust/jamf-integration)
