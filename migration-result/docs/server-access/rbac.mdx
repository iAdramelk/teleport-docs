...
For a more general description of Teleport roles and examples see our [Access Controls guides](/docs/access-controls/introduction), as
...
Variables with the format `{{external.xyz}}` are replaced with values from external [SSO](/docs/access-controls/sso)
