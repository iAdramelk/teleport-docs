...
As you learned in [Core Concepts](/docs/core-concepts), a Teleport cluster
...
through an external identity provider, see [Configure Single Sign-on](/docs/access-controls/sso).
...
- The `tctl` admin tool and `tsh` client tool version >= 15.0.2.
  ...
  For more information about installing Teleport, see [Installation](/docs/installation).
...
- A Teleport SSH server that is joined to the cluster you plan to use as the **leaf cluster**.
  ...
  [Join Services to your Cluster](/docs/agents/join-services-to-your-cluster).
