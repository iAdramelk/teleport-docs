...
See our [Upgrading](/docs/upgrading) guide for more information.
...
[Using Teleport Connect](/docs/connect-your-client/teleport-connect) for usage and installation*.
...
| ------------------------------------------------------------------------------ | ------------- | ------------------------ | -------- | ----------- | ---------------------- | -------------- |
| [Enhanced Session Recording](/docs/server-access/guides/bpf-session-recording) | Kernel  v5.8+ | 11, or 10 with backports | 20.04.2+ | 9+          | 2 (post 11/2021), 2023 | 12 SP5, 15 SP5 |
...
<Tabs>
  ...
  <Tab title="Teleport Enterprise Cloud">
    Check the [Cloud Downloads](/docs/choose-an-edition/teleport-cloud/downloads) page for the most up-to-date
...
<Tabs>
  <Tab title="Teleport Community Edition">
    ...
    | --------------------------------------------------------------- | ---------------------- | -------------------------------------------------------------------------- |
    | `public.ecr.aws/gravitational/teleport-distroless:15.0.2`       | No                     | [Distroless Debian 12](https://github.com/GoogleContainerTools/distroless) |
    | `public.ecr.aws/gravitational/teleport-distroless-debug:15.0.2` | Yes                    | [Distroless Debian 12](https://github.com/GoogleContainerTools/distroless) |

    For testing, we always recommend that you use the latest release version of
    Teleport, which is currently `public.ecr.aws/gravitational/teleport-distroless:15.0.2`.
  </Tab>

  <Tab title="Teleport Enterprise">
    | Image name                                                          | Includes troubleshooting tools | Image base                                                                 |
    | ------------------------------------------------------------------- | ------------------------------ | -------------------------------------------------------------------------- |
    | `public.ecr.aws/gravitational/teleport-ent-distroless:15.0.2`       | No                             | [Distroless Debian 12](https://github.com/GoogleContainerTools/distroless) |
    | `public.ecr.aws/gravitational/teleport-ent-distroless-debug:15.0.2` | Yes                            | [Distroless Debian 12](https://github.com/GoogleContainerTools/distroless) |

    We also provide the following images for FIPS builds of Teleport Enterprise:

    | Image name                                                               | Includes troubleshooting tools | Image base                                                                 |
    | ------------------------------------------------------------------------ | ------------------------------ | -------------------------------------------------------------------------- |
    | `public.ecr.aws/gravitational/teleport-ent-fips-distroless:15.0.2`       | No                             | [Distroless Debian 12](https://github.com/GoogleContainerTools/distroless) |
    | `public.ecr.aws/gravitational/teleport-ent-fips-distroless-debug:15.0.2` | Yes                            | [Distroless Debian 12](https://github.com/GoogleContainerTools/distroless) |

    For testing, we always recommend that you use the latest release version of
    Teleport Enterprise, which is currently `public.ecr.aws/gravitational/teleport-ent-distroless:15.0.2`.
  </Tab>

  <Tab title="Teleport Team/Teleport Enterprise Cloud">
    | Image name                                                          | Includes troubleshooting tools | Image base                                                                 |
    | ------------------------------------------------------------------- | ------------------------------ | -------------------------------------------------------------------------- |
    | `public.ecr.aws/gravitational/teleport-ent-distroless:14.3.6`       | No                             | [Distroless Debian 12](https://github.com/GoogleContainerTools/distroless) |
    | `public.ecr.aws/gravitational/teleport-ent-distroless-debug:14.3.6` | Yes                            | [Distroless Debian 12](https://github.com/GoogleContainerTools/distroless) |

    For testing, we always recommend that you use the latest Cloud release version of
    Teleport Enterprise, which is currently `public.ecr.aws/gravitational/teleport-ent-distroless:14.3.6`.
  </Tab>
</Tabs>
...
```bash
$ docker run -it --entrypoint="" public.ecr.aws/gravitational/teleport-distroless -debug:public.ecr.aws/gravitational/teleport-distroless:15.0.2 busybox sh
```
...
See the Teleport [Configuration Reference](/docs/reference/config) for whether a
...
Reference](/docs/reference/networking#ports) for the ports on your Teleport
container to expose.
...
method](/docs/agents/join-services-to-your-cluster/join-token).
...
$ docker run --hostname localhost --rm \
  ...
  public.ecr.aws/gravitational/teleport-distroless:15.0.2 configure --roles=proxy,auth > ~/teleport/config/teleport.yaml
```
...
$ docker run --hostname localhost --name teleport \
  ...
  public.ecr.aws/gravitational/teleport-distroless:15.0.2
```
...
3. Run a new container with an image based on a newer Teleport version, mounting
...
[reference Terraform code](https://github.com/gravitational/teleport/tree/branch/v15/examples/aws/terraform).

See the [AWS Single-Instance Deployment](/docs/deploy-a-cluster/deployments/aws-starter-cluster-terraform)
and the [Running Teleport Enterprise in High Availability mode on AWS using Terraform](/docs/deploy-a-cluster/deployments/aws-ha-autoscale-cluster-terraform)
...
`/etc/teleport.d/conf` file on the EC2 instance. See the [Starter Cluster Configuration Template](https://github.com/gravitational/teleport/tree/branch/v15/examples/aws/terraform/starter-cluster/data.tpl)
...
| -------------------------------------------- | ---------- | ------------ | ------------ | ------------------------------------------------------------------------------- |
| `teleport-oss-15.0.2-x86_64-$TIMESTAMP`      | OSS        | amd64        | No           | `TeleportVersion: 15.0.2`, `TeleportEdition: oss`, `TeleportFipsEnabled: false` |
| `teleport-oss-15.0.2-arm64-$TIMESTAMP`       | OSS        | arm64        | No           | `TeleportVersion: 15.0.2`, `TeleportEdition: oss`, `TeleportFipsEnabled: false` |
| `teleport-ent-15.0.2-x86_64-$TIMESTAMP`      | Enterprise | amd64        | No           | `TeleportVersion: 15.0.2`, `TeleportEdition: ent`, `TeleportFipsEnabled: false` |
| `teleport-ent-15.0.2-arm64-$TIMESTAMP`       | Enterprise | arm64        | No           | `TeleportVersion: 15.0.2`, `TeleportEdition: ent`, `TeleportFipsEnabled: false` |
| `teleport-ent-15.0.2-x86_64-fips-$TIMESTAMP` | Enterprise | amd64        | Yes          | `TeleportVersion: 15.0.2`, `TeleportEdition: ent`, `TeleportFipsEnabled: true`  |
| `teleport-ent-15.0.2-arm64-fips-$TIMESTAMP`  | Enterprise | arm64        | Yes          | `TeleportVersion: 15.0.2`, `TeleportEdition: ent`, `TeleportFipsEnabled: true`  |
...
| --------------------- | -------------------------------------------------------------------------------------------- | --------------------------------------------------------------- |
| `teleport-cluster`    | Auth Service<br />Proxy Service<br />Other Teleport services if using a custom configuration | [Reference](/docs/reference/helm-reference/teleport-cluster)    |
| `teleport-kube-agent` | Kubernetes Service<br />Application Service<br />Database Service                            | [Reference](/docs/reference/helm-reference/teleport-kube-agent) |

## macOS

<Tabs dropdownView dropdownCaption="Teleport Edition">
  <Tab title="Community/Team">
    <Tabs>
      <Tab title="Teleport package">
        ...
        | Link                                                                  | Binaries                                      |
        | --------------------------------------------------------------------- | --------------------------------------------- |
        | [`teleport-15.0.2.pkg`](https://cdn.teleport.dev/teleport-15.0.2.pkg) | `teleport`<br />`tctl`<br />`tsh`<br />`tbot` |
        | [`tsh-15.0.2.pkg`](https://cdn.teleport.dev/tsh-15.0.2.pkg)           | `tsh`                                         |

        You can also fetch an installer via the command line:

        ```bash
        $ curl -O https://cdn.teleport.dev/teleport-15.0.2.pkg
        # Installs on Macintosh HD
        $ sudo installer -pkg teleport-15.0.2.pkg -target /
        # Password:
        # installer: Package name is teleport-15.0.2
      ...
      <Tab title="Homebrew">
        ...
        [compatibility policy](/docs/upgrading) for details.
  ...
  <Tab title="Enterprise">
    ...
    | Link                                                                          | Binaries                                      |
    | ----------------------------------------------------------------------------- | --------------------------------------------- |
    | [`teleport-ent-15.0.2.pkg`](https://cdn.teleport.dev/teleport-ent-15.0.2.pkg) | `teleport`<br />`tctl`<br />`tsh`<br />`tbot` |
    | [`tsh-15.0.2.pkg`](https://cdn.teleport.dev/tsh-15.0.2.pkg)                   | `tsh`                                         |

    You can also fetch an installer from the command line:

    ```bash
    $ curl -O https://cdn.teleport.dev/teleport-ent-15.0.2.pkg
    # Install on Macintosh HD
    $ sudo installer -pkg teleport-ent-15.0.2.pkg -target / 
    Password:
    installer: Package name is teleport-ent-15.0.2
    installer: Upgrading at base path /
    installer: The upgrade was successful.
    $ which teleport
    /usr/local/bin/teleport
  ...
  <Tab title="Enterprise Cloud">
    ...
    | Link                                                                          | Binaries                                      |
    | ----------------------------------------------------------------------------- | --------------------------------------------- |
    | [`teleport-ent-14.3.6.pkg`](https://cdn.teleport.dev/teleport-ent-14.3.6.pkg) | `teleport`<br />`tctl`<br />`tsh`<br />`tbot` |
    | [`tsh-14.3.6.pkg`](https://cdn.teleport.dev/tsh-14.3.6.pkg)                   | `tsh`                                         |

    You can also fetch an installer from the command line:

    ```bash
    $ curl -O https://cdn.teleport.dev/teleport-ent-14.3.6.pkg
    # Install on Macintosh HD
    $ sudo installer -pkg teleport-ent-14.3.6.pkg -target / 
    Password:
    installer: Package name is teleport-ent-14.3.6
    installer: Upgrading at base path /
    installer: The upgrade was successful.
    $ which teleport
    /usr/local/bin/teleport
...
<Tabs dropdownView dropdownCaption="Teleport Edition">
  <Tab title="Teleport Community Edition">
    ...
    $ $Resp = Invoke-WebRequest https://cdn.teleport.dev/teleport-v15.0.2-windows-amd64-bin.zip.sha256
    ...
    # <checksum> <filename>
    $ Invoke-WebRequest -OutFile teleport-v15.0.2-windows-amd64-bin.zip -Uri https://cdn.teleport.dev/teleport-v15.0.2-windows-amd64-bin.zip
    $ certUtil -hashfile teleport-v15.0.2-windows-amd64-bin.zip SHA256
    # SHA256 hash of teleport-v15.0.2-windows-amd64-bin.zip:
    # <checksum>
    ...
    `teleport-v15.0.2-windows-amd64-bin\tsh.exe`.

    ```bash
    $ Expand-Archive teleport-v15.0.2-windows-amd64-bin.zip
    $ cd teleport-v15.0.2-windows-amd64-bin
    $ .\tsh.exe version
    Teleport v15.0.2 git:v15.0.2 go1.21
    ```
  ...
  <Tab title="Teleport Enterprise Cloud">
    ...
    $ [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
    # Get the expected checksum for the Windows tsh package
    $ $Resp = Invoke-WebRequest https://cdn.teleport.dev/teleport-v14.3.6-windows-amd64-bin.zip.sha256
    # PowerShell will return the binary representation of the response content
    # by default, so you need to convert it to a string
    $ [System.Text.Encoding]::UTF8.getstring($Resp.Content)
    # <checksum> <filename>
    $ Invoke-WebRequest -OutFile teleport-v14.3.6-windows-amd64-bin.zip -Uri https://cdn.teleport.dev/teleport-v14.3.6-windows-amd64-bin.zip
    $ certUtil -hashfile teleport-v14.3.6-windows-amd64-bin.zip SHA256
    # SHA256 hash of teleport-v14.3.6-windows-amd64-bin.zip:
    # <checksum>
    # CertUtil: -hashfile command completed successfully.
    ```
    ...
    After you have verified that the checksums match, you can extract the archive.
    The executable will be available at
    `teleport-v14.3.6-windows-amd64-bin\tsh.exe`.

    ```bash
    $ Expand-Archive teleport-v14.3.6-windows-amd64-bin.zip
    $ cd teleport-v14.3.6-windows-amd64-bin
    $ .\tsh.exe version
    Teleport v14.3.6 git:v14.3.6 go1.21
    ```
...
Teleport is written in Go, and currently requires **go v1.21** or
...
<img src="/assets/teleport-sha-81ff1209cc.png" alt="Teleport Checksum" width="2404" height="1088" />
...
If you wish to uninstall Teleport at any time, see our documentation on [Uninstalling Teleport](/docs/management/admin/uninstall-teleport).
...
- [Database Access](/docs/database-access/introduction)
- [Application Access](/docs/application-access/introduction)
- [Desktop Access](/docs/desktop-access/introduction)
- [Machine ID](/docs/machine-id/introduction)
