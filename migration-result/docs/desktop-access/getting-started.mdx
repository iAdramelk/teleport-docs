...
If you have more than 5 desktops registered under `non_ad_hosts`, Teleport
...
3. Download the Teleport Windows Auth [setup program](https://cdn.teleport.dev/teleport-windows-auth-setup-v15.0.0-dev-amd64.exe).
...
5. Configure the `/etc/teleport.yaml` file on the Linux server with settings similar to the
   ...
   - List the Windows IP addresses or host names under `non_ad_hosts`.
...
6. (Optional) Add labels for the Windows Desktop Service to the configuration file.
   ...
   You can attach labels to Windows computers by matching to their hostnames.
   ...
   role permissions](/docs/desktop-access/rbac).
...
2. Create a `windows-desktop-admins.yaml` file to define a new role:
   ...
   to create local Windows logins, see [Logins](/docs/desktop-access/rbac#logins).
...
Getting Started](/docs/access-controls/getting-started#step-13-add-local-users-with-preset-roles).
