...
Active Directory with scripts](/docs/desktop-access/active-directory). However, if automation
...
- Access to a running Teleport cluster, `tctl` admin tool, and `tsh` client tool
  ...
  instructions](/docs/installation#installation-instructions) for the Teleport
  edition you use.
...
- An Active Directory domain that is configured for LDAPS. Because Teleport requires an
  ...
  Active Directory with scripts](/docs/desktop-access/active-directory) for how to
...
7. Verify the **Teleport Service Account** is selected, then click **OK** in all the dialogs.
   ...
   <img src="/assets/deny-interactive-login-a3c84c9ae7.png" alt="Deny interactive login" width="2880" height="1800" />
...
1. Create a new group policy object with the name `Teleport Access Policy` by running the following
   ...
   <img src="/assets/aws-managed-ad-3a5a0f8fb4.png" alt="AWS Managed AD OU Location" width="1325" height="622" />
...
4. In the group policy editor, expand
...
7. To ensure your GPO update takes effect immediately on this host,
...
5. To ensure your GPO update takes effect immediately on this host,
   open PowerShell and run the following command (optional):
...
5. Right-click **Require user authentication for remote connections by using
   ...
   <img src="/assets/disable-614eb85f76.png" alt="Disable Require" width="2880" height="1800" />
...
8. Right-click **Inbound Rules**, select **New Rule**.
   ...
   <img src="/assets/firewall-956a974a16.png" alt="Open the Firewall" width="2880" height="1800" />

9. To ensure your GPO update takes effect immediately on this host,
   open PowerShell and run the following command (optional):
...
4. Now left-click **Remote Session Environment**
   ...
   <img src="/assets/enable-remotefx-004370adf5.png" alt="Enable RemoteFX" width="2560" height="1284" />
...
3. Right-click **Server authentication certificate template**, select **Edit**, select
   **Enabled**, then set the Certificate Template Name to **RemoteDesktopAccess**.
   ...
   <img src="/assets/rdp-certificate-template-23bc1271e3.png" alt="RDP Certificate Template" width="2746" height="1648" />
...
5. Add the configuration for the Windows Desktop Service to the `/etc/teleport.yaml`
   ...
   [Desktop Configuration Reference](/docs/desktop-access/reference/configuration).
...
2. Configure Teleport to perform PKI against the root domain, while
...
If you encounter any issues, see [Troubleshooting](/docs/desktop-access/troubleshooting) for common problems and
solutions.
For information about configuring Windows-specific role permissions, see
[Role-Based Access Control for Desktops](/docs/desktop-access/rbac).
