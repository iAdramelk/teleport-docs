...
be automatically [locked](/docs/access-controls/guides/locking).
...
When requesting [Database Access](/docs/database-access/introduction)
...
Teleport [RBAC permissions](/docs/access-controls/reference) and are not
allowed to view or list databases by default unless granted permission via one
or more roles.

### Resolution

Per the [Machine ID Database Access Guide](/docs/machine-id/access-guides/databases), ensure at
