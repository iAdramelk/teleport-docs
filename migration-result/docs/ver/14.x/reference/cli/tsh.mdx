...
| TELEPORT\_LOGIN\_BROWSER         | Set to `none` to stop the system default browser from opening for SSO logins. If the value is not `none`, `tsh` will open the system default browser. | none                     |
...
See [full docs on `tsh puttyconfig` here](/docs/ver/14.x/connect-your-client/putty-winscp).
