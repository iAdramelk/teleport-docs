...
- [Auth](/docs/ver/14.x/architecture/authentication)
- [Node/SSH](/docs/ver/14.x/architecture/nodes)
- [Proxy](/docs/ver/14.x/architecture/proxy)
- [App](/docs/ver/14.x/application-access/introduction)
- [Database](/docs/ver/14.x/database-access/introduction)
- [Windows Desktop](/docs/ver/14.x/desktop-access/introduction)
- [Kubernetes](/docs/ver/14.x/kubernetes-access/introduction)
...
| `teleport start`                       | Starts the `teleport` process in the foreground using the current shell session, including any services configured by the [configuration YAML file](/docs/ver/14.x/reference/config).                                                                    |
| `teleport status`                      | Prints the status of the current active Teleport SSH session.                                                                                                                                                                                            |
| `teleport configure`                   | Generates and writes a [configuration YAML file](/docs/ver/14.x/reference/config) for the Teleport service. This file should be customized in production to suit the needs of your environment, and the default output should only be used when testing. |
...
we recommend using a [configuration file](/docs/ver/14.x/reference/config) in production.
...
| `-r, --roles`          | `proxy`, `node`, `auth`                                 | **string** comma-separated list of `proxy`, `node`, `auth`, `db`, or `app` | start listed services/roles. These roles are explained in the [Core Concepts](/docs/ver/14.x/core-concepts) document.                                                                                                                                         |
...
| `--labels`             | none                                                    | **string** comma-separated list                                            | assigns a set of labels to a node, for example env=dev,app=web. See the explanation of labeling mechanism in the [Labeling Nodes](/docs/ver/14.x/management/admin/labels) section.                                                                            |
...
| ----------------------------------------------------- | --------- | -------------------------------------------------------------------------------------------------------------------------- |
| [Node](/docs/ver/14.x/architecture/nodes)             | `node`    | Allows SSH connections from authenticated clients.                                                                         |
| [Auth](/docs/ver/14.x/architecture/authentication)    | `auth`    | Authenticates and authorizes hosts and users who want access to Teleport-managed resources or information about a cluster. |
| [Proxy](/docs/ver/14.x/architecture/proxy)            | `proxy`   | The gateway that clients use to connect to the Auth Service or resources managed by Teleport.                              |
| [App](/docs/ver/14.x/application-access/introduction) | `app`     | Provides access to applications.                                                                                           |
| [Database](/docs/ver/14.x/database-access/reference)  | `db`      | Provides access to databases.                                                                                              |
...
| `--auth`               | `local`          | Any defined [authentication connector](/docs/ver/14.x/reference/authentication), including `passwordless` and `local` (i.e., no authentication connector) | Specify the type of authentication connector to use.                                                                                                                                      |
...
```code
$ tsh version
Teleport v14.0.0-dev git: go1.20
Proxy version: 14.0.0-dev
...
| Name        | Default Value(s) | Allowed Value(s) | Description                    |
| ----------- | ---------------- | ---------------- | ------------------------------ |
| `--cluster` | none             | a cluster\_name  | Specify the cluster to connect |
...
<Tip>
  ...
  in their [storage backend](/docs/ver/14.x/reference/backends).
...
through Teleport.  See the [OpenSSH Guide](/docs/ver/14.x/server-access/guides/openssh) guide
...
Access](/docs/ver/14.x/application-access/cloud-apis/azure).
...
- `<cluster>` - the name of the cluster,  see [Trusted Cluster](/docs/ver/14.x/management/admin/trustedclusters) for more information.
...
Log into a Kubernetes cluster. Discover connected clusters by using [`tsh kube ls`](/docs/ver/14.x/reference/cli#tsh-kube-ls).
...
registered devices using [`tsh mfa ls`](/docs/ver/14.x/reference/cli#tsh-mfa-ls).
...
| `--query`  | none             | String                                                       | Query by [predicate language](/docs/ver/14.x/reference/predicate-language#resource-filtering) enclosed in single quotes |
...
| -------------------------------- | --------------------------------------------------------------------------- | ------------------------ |
| TELEPORT\_AUTH                   | Name of a defined SAML, OIDC, or GitHub auth connector (or a local user)    | okta                     |
| TELEPORT\_MFA\_MODE              | Preferred mode for MFA and Passwordless assertions                          | otp                      |
| TELEPORT\_CLUSTER                | Name of a Teleport root or leaf cluster                                     | cluster.example.com      |
| TELEPORT\_LOGIN                  | Login name to be used by default on the remote host                         | root                     |
| TELEPORT\_LOGIN\_BIND\_ADDR      | Address in the form of host:port to bind to for login command webhook       | host:port                |
| TELEPORT\_PROXY                  | Address of the Teleport proxy server                                        | cluster.example.com:3080 |
| TELEPORT\_HOME                   | Home location for tsh configuration and data                                | /directory               |
| TELEPORT\_USER                   | A Teleport user name                                                        | alice                    |
| TELEPORT\_ADD\_KEYS\_TO\_AGENT   | Specifies if the user certificate should be stored on the running SSH agent | yes, no, auto, only      |
| TELEPORT\_USE\_LOCAL\_SSH\_AGENT | Disable or enable local SSH agent integration                               | true, false              |
| TELEPORT\_GLOBAL\_TSH\_CONFIG    | Override location of global `tsh` config file from default `/etc/tsh.yaml`  | /opt/teleport/tsh.yaml   |
| TELEPORT\_HEADLESS               | Use headless authentication                                                 | true, false, 1, 0        |
...
<Tabs>
  <Tab title="Self-Hosted">
    ...
    <Note>
      ...
      [Access Controls](/docs/ver/14.x/access-controls/introduction) and ensure that
...
<Tip>
  ...
  [Machine ID](/docs/ver/14.x/machine-id/introduction) is a more secure alternative that
...
| `--google-legacy`  | Flag to select groups with direct membership filtered by domain (legacy behavior). <br />Disabled by default. [More info](/docs/ver/14.x/access-controls/sso/google-workspace#how-teleport-uses-google-workspace-apis) |
...
See the [Resources Reference](/docs/ver/14.x/reference/resources) for complete docs on how to build these yaml files.
...
The `--query` flag allows you to perform more sophisticated searches using a [predicate language](/docs/ver/14.x/reference/predicate-language#resource-filtering).
...
Refer to the [database guide](/docs/ver/14.x/machine-id/guides/databases) for more information on
