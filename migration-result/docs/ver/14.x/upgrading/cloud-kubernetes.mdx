...
](/docs/ver/14.x/architecture/agent-update-management) guide describes how agent
...
2. Add the following chart values to your existing agent `values.yaml` to enable
...
3. Upgrade the Helm chart release with the new values by running `helm upgrade`.
