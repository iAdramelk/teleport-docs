...
<Tip>
  ...
  [Choose an Edition](/docs/ver/14.x/choose-an-edition/introduction).
</Tip>
...
- The `tsh` client tool.
  ...
  For installation instructions, see [Installation Guide](/docs/ver/14.x/installation).
...
2. Sign in with the account credentials you used to activate your Teleport Team account.
   ...
   <img src="/assets/add-my-first-resource@2x-239d908158.png" alt="Add your first resource" width="2560" height="1440" />

3. Click **Add my first resource** to open **Enroll New Resource**.

4. Type *server* in the search box to filter the list of resources.

   <img src="/assets/choose-resource@2x-e0b1d4cb7e.png" alt="Select resource type" width="1280" height="720" />
...
5. Click *Ubuntu 14.04+* to register the server in the Docker container.
   ...
   <img src="/assets/paste-script@2x-0b28e03cd9.png" alt="Configure resource" width="1280" height="720" />
...
6. In the terminal with the container shell session, alias `sudo` by running
...
7. Copy the command to run the installation script from the Teleport Web UI and paste it
   ...
   <img src="/assets/successfully-connected@2x-56f5ff8ca4.png" alt="Connected successfully" width="1934" height="891" />

   To continue, click **Next**.

8. Confirm that you want to authenticate as the `root` user and click **Next**:

   <img src="/assets/set-up-access@2x-71e027aeb3.png" alt="Choose your user" width="1280" height="720" />

9. Click **Test Connection** to verify access to the server.

   <img src="/assets/test-connection@2x-1182bd3e8c.png" alt="Start session" width="1280" height="720" />
...
3. Enter the `exit` command to end the remote interactive session and close the browser tab.
...
2. Under Activity, click **Session Recordings**.

   You will see the recording for your interactive session from the previous step listed.
   For example:

   <img src="/assets/session-recordings@2x-cb38decb3f.png" alt="Session recordings" width="1280" height="720" />
...
2. Sign in to your Teleport Team cluster by running the `tsh login` command  with the URL
   ...
   ```bash
   $ tsh login --proxy=mytenant.teleport.sh --user=username
   ```
...
3. List the servers your Teleport user can access.
   ...
   ------------ ---------- ----------------------------------------------------------------------------------------
   node-name ⟵ Tunnel   hostname=node-name
   ```
...
with Terraform](/docs/ver/14.x/agents/deploy-agents-terraform).
