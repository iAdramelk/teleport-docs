...
<Tabs>
  <Tab title="Console">
    ...
    ![Custom role](/assets/custom-role@2x-c52bea0d2b.png)
    ...
    ![Service account](/assets/create-service-account@2x-92b9dbcc35.png)
    ...
    ![Role assignment](/assets/assign-role-to-service-account@2x-a04dd2bae3.png)
  </Tab>
...
- `{{ .RepoChannel }}`: Optional package repository (apt/yum) channel name.
  Has format `<channel>/<version>` e.g. stable/v14. See [installation](/docs/ver/14.x/installation#linux) for more details.
- `{{ .AutomaticUpgrades }}`: indicates whether Automatic Upgrades are enabled or disabled.
  Its value is either `true` or `false`. See
  [self-hosted automatic agent updates](/docs/ver/14.x/upgrading/self-hosted-automatic-agent-updates)
...
echo Teleport-14.3.6
echo Repository Channel: stable/v14.3.6
```
...
- Read [Joining Services via GCP](/docs/ver/14.x/agents/join-services-to-your-cluster/gcp)
  for more information on GCP tokens.
- Full documentation on GCP discovery configuration can be found through the [
  config file reference documentation](/docs/ver/14.x/reference/config).
- The complete default installer can be found [with the Teleport source
  ](https://github.com/gravitational/teleport/blob/branch/v14/api/types/installers/installer.sh.tmpl).
