...
<Accordion title="Using another join method">
  ...
  - Google Kubernetes Engine: Where
    ...
    See the [GCP Platform Guide](/docs/ver/14.x/machine-id/deployment/gcp) for further information.
  - AWS Elastic Kubernetes Service: Where
    ...
    See the [AWS Platform Guide](/docs/ver/14.x/machine-id/deployment/aws) for further information.
</Accordion>
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: tbot
          image: public.ecr.aws/gravitational/teleport:14.3.6
          command:
            - tbot
          args:
            - start
...
Follow one of the [access guides](/docs/ver/14.x/machine-id/access-guides) to configure an output
...
- Follow the [access guides](/docs/ver/14.x/machine-id/access-guides) to finish configuring `tbot` for
  your environment.
