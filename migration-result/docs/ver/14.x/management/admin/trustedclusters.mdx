...
As you learned in [Core Concepts](/docs/ver/14.x/core-concepts), a Teleport cluster
...
<img src="/assets/simplified-trusted-cluster-d29e680b9e.png" alt="Trusted cluster" width="2784" height="1050" />
...
<img src="/assets/trusted-cluster-service-interaction-64108e5493.png" alt="Service interaction in a trusted cluster" width="1480" height="915" />
...
<img src="/assets/simplified-trusted-cluster-role-mapping-2478ae52be.png" alt="Role mapping in a trusted cluster" width="1800" height="1235" />
...
through an external identity provider, see [Configure Single Sign-on](/docs/ver/14.x/access-controls/sso).
...
- The `tctl` admin tool and `tsh` client tool version >= 14.3.6.
  ...
  For more information about installing Teleport, see [Installation](/docs/ver/14.x/installation).
...
- A Teleport SSH server that is joined to the cluster you plan to use as the **leaf cluster**.
  ...
  [Join Services to your Cluster](/docs/ver/14.x/agents/join-services-to-your-cluster).
...
2. Add the local `visitor` user and create a home directory for the user by running the
   following command:
...
4. Sign in to your **leaf cluster** from your administrative workstation using
...
2. Sign in to your **root cluster** from your administrative workstation using
   your Teleport username:
...
3. Create a file called `user.yaml` with your current user configuration by running
...
4. Open the `user.yaml` file in a text editor and add the `visitor` login:
...
5. Apply your changes by running the following command:
...
4. Create a resource configuration file called `trusted_cluster.yaml` with the
   following content:
...
2. Create the trusted cluster resource from the resource configuration file by running
...
4. Create a role configuration file called `demo-cluster-access.yaml` that
...
6. Assign the `demo-cluster-access` role to your Teleport user by running the appropriate
   ...
   <Tabs>
     <Tab title="Local User">
       ...
       2. Edit `out.yaml`, adding `demo-cluster-access` to the list of existing roles:
     ...
     <Tab title="GitHub">
       ...
       2. Edit `github.yaml`, adding `demo-cluster-access` to the `teams_to_roles` section.
     ...
     <Tab title="SAML">
       ...
       2. Edit `saml.yaml`, adding `demo-cluster-access` to the `attributes_to_roles` section.
     ...
     <Tab title="OIDC">
       ...
       2. Edit `oidc.yaml`, adding `demo-cluster-access` to the `claims_to_roles` section.
...
7. Confirm that the leaf cluster has been updated with the label you set by
   running the following command:
...
2. Confirm that the server running the Teleport agent is joined to the leaf cluster by
...
4. Confirm you are logged in with as the user `visitor` on the server
...
2. Retrieve the resource configuration by running the following command:
...
4. Update the trusted cluster configuration by running the following command:
...
3. Sign in to your **root cluster** from your administrative workstation using
   your Teleport username:
...
4. Delete the certificate authorities associated with the remote cluster and remove the
