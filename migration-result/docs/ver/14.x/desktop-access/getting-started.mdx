...
If you have more than 5 desktops registered under `non_ad_hosts` and `static_hosts` with `ad: false`, Teleport
...
5. Configure the `/etc/teleport.yaml` file on the Linux server with settings similar to the
   ...
   - List the Windows desktops under `static_hosts` (or IP addresses or host names under `non_ad_hosts` in versions prior to 14.3).
...
6. (Optional) Add labels for the Windows Desktop Service to the configuration file.
   ...
   In Teleport 14.3 and later, you can add labels to Windows computers using the `labels` section of the host config. For example, the following
   ...
   windows_desktop_service:
     ...
     non_ad_hosts:
   -    - 192.0.2.155
   +    - ip-192-0-2-155.us-east-2.compute.internal
     static_hosts:
      - name: host1
        ad: false
   -    addr: 192.0.2.156
   ...
   role permissions](/docs/ver/14.x/desktop-access/rbac).
...
2. Create a `windows-desktop-admins.yaml` file to define a new role:
   ...
   to create local Windows logins, see [Logins](/docs/ver/14.x/desktop-access/rbac#logins).
...
Getting Started](/docs/ver/14.x/access-controls/getting-started#step-13-add-local-users-with-preset-roles).
