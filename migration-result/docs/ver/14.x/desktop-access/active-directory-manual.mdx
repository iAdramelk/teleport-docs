...
Active Directory with scripts](/docs/ver/14.x/desktop-access/active-directory). However, if automation
...
- Access to a running Teleport cluster, `tctl` admin tool, and `tsh` client tool
  ...
  instructions](/docs/ver/14.x/installation#installation-instructions) for the Teleport
  edition you use.
...
- An Active Directory domain that is configured for LDAPS. Because Teleport requires an
  ...
  Active Directory with scripts](/docs/ver/14.x/desktop-access/active-directory) for how to
...
5. Add the configuration for the Windows Desktop Service to the `/etc/teleport.yaml`
   ...
   [Desktop Configuration Reference](/docs/ver/14.x/desktop-access/reference/configuration).
...
If you encounter any issues, see [Troubleshooting](/docs/ver/14.x/desktop-access/troubleshooting) for common problems and
solutions.
