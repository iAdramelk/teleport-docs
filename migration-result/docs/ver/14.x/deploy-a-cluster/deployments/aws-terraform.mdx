...
Setting `export TF_VAR_ami_name="gravitational-teleport-ami-ent-14.0.0-dev"`
...
where `<type>` is either `oss` or `ent` (Enterprise) and `version` is the version of Teleport e.g. `14.0.0-dev`.
...
- 1 x m4.large Teleport **node** instance in an ASG. Server instances in this ASG will join with Teleport SSH Service enabled and connected to the **auth**
  service. [The desired size of the ASG is configured here](https://github.com/gravitational/teleport/blob/master/examples/aws/terraform/ha-autoscale-cluster/node_asg.tf#L10)
...
You can also generate a Node join token using `tctl tokens add --type=node` [as detailed here in our admin guide](/docs/ver/14.x/agents/join-services-to-your-cluster/join-token).
...
for this document) or join your nodes using [Teleport's node tunneling](/docs/ver/14.x/agents/join-services-to-your-cluster/join-token) functionality.
...
to an auth server. Follow the instructions in our [Trusted Clusters guide](/docs/ver/14.x/management/admin/trustedclusters).
