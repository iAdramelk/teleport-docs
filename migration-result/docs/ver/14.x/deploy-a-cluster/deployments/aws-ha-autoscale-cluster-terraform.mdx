...
This guide is designed to accompany our [reference Terraform code](https://github.com/gravitational/teleport/tree/branch/v14/examples/aws/terraform/ha-autoscale-cluster#terraform-based-provisioning-example-amazon-single-ami)
...
```bash
$ git clone https://github.com/gravitational/teleport -b branch/v14
...
`examples/aws/terraform/ha-autoscale-cluster` section of the Teleport repo](https://github.com/gravitational/teleport/blob/branch/v14/examples/aws/terraform/ha-autoscale-cluster/README.md)
...
The AWS region to run in. You should pick from the supported list as detailed in the [README](https://github.com/gravitational/teleport/blob/branch/v14/examples/aws/terraform/ha-autoscale-cluster/README.md).
...
```bash
$ export TF_VAR_ami_name="gravitational-teleport-ami-ent-14.3.6"
```
...
where `<type>` is either `oss` or `ent` (Enterprise) and `version` is the version of Teleport e.g. `14.3.6`.
...
<Tip>
  <p>
    **List Gravitational AMIs**
  </p>

  OSS AMIs<br />

  ```bash
  $ aws --region us-west-2 ec2 describe-images --owners 126027368216 --filters 'Name=name,Values=gravitational-teleport-ami-oss-14.3.6'
  ```

  Enterprise AMIs<br />

  ```bash
  $ aws --region us-west-2 ec2 describe-images --owners 126027368216 --filters 'Name=name,Values=gravitational-teleport-ami-ent-14.3.6'
  ```

  List Enterprise FIPS 140-2 AMIs<br />

  ```bash
  $ aws --region us-west-2 ec2 describe-images --owners 126027368216 --filters 'Name=name,Values=gravitational-teleport-ami-ent-14.3.6-fips'
  ```
</Tip>
...
If set to the string `true`, Teleport will use [TLS routing](/docs/ver/14.x/architecture/tls-routing) to multiplex all traffic over a single port.
...
See the [Teleport authentication reference](/docs/ver/14.x/reference/authentication) for more information.
...
- 2 x `m4.large` Teleport Auth Service instances in an ASG, behind an internal network load balancer, configured using DynamoDB for
  shared storage. [The desired size of the ASG is configured here](https://github.com/gravitational/teleport/blob/branch/v14/examples/aws/terraform/ha-autoscale-cluster/auth_asg.tf#L11)
- 2 x `m4.large` Teleport Proxy Service instances in an ASG, behind a public-facing load balancer - NLB for LetsEncrypt, ALB for ACM. [The desired size of the ASG is configured here](https://github.com/gravitational/teleport/blob/branch/v14/examples/aws/terraform/ha-autoscale-cluster/proxy_asg.tf#L12)
- 1 x `m4.large` Teleport SSH Service instance in an ASG. Server instances in this ASG will join with Teleport SSH Service enabled and connected to the **auth**
  service. [The desired size of the ASG is configured here](https://github.com/gravitational/teleport/blob/branch/v14/examples/aws/terraform/ha-autoscale-cluster/node_asg.tf#L10)
- 1 x `m4.large` monitoring server in an ASG which hosts the Grafana instance and receives monitoring data from each service in the cluster. [The desired size of the ASG is configured here](https://github.com/gravitational/teleport/blob/branch/v14/examples/aws/terraform/ha-autoscale-cluster/monitor_asg.tf#L12)
...
[The instance types used for each ASG can be configured here](https://github.com/gravitational/teleport/blob/branch/v14/examples/aws/terraform/ha-autoscale-cluster/vars.tf#L23-L45)
...
More information about how Teleport works with DynamoDB can be found in our [Storage Backends guide](/docs/ver/14.x/reference/backends#dynamodb).
...
by the [`route53_domain`](#route53_domain) variable. In our example this would be `<Var name="teleport.example.com" />`
...
<Tabs>
  <Tab title="With TLS routing">
    ...
    on the same hostname as set in [`route53_domain`](#route53_domain).
  </Tab>

  <Tab title="Without TLS routing">
...
2. Get the IP of an Auth Service instance by using `terraform output -json auth_instance_private_ips` and piping to `awk` to extract the first IP.
...
3. Use both these values to SSH into the Auth Service instance. Make sure that the AWS keypair that you specified in the
...
5. Click the link to launch the Teleport web UI and finish setting up your user. You can choose whether to use a WebAuthn-compatible hardware key
...
<Tabs>
  <Tab title="If using Let's Encrypt">
...
ways to integrate Teleport onto your servers. We recommend looking at our [Installation guide](/docs/ver/14.x/installation).

To add new nodes/EC2 servers that you can "SSH into" you'll need to:

- [Install the Teleport binary on the Server](/docs/ver/14.x/installation)
- [Run Teleport - we recommend using systemd](/docs/ver/14.x/management/admin/daemon)
- [Set the correct settings in /etc/teleport.yaml](/docs/ver/14.x/reference/config)
- [Add Nodes to the Teleport
  cluster](/docs/ver/14.x/agents/join-services-to-your-cluster)
...
$ aws ssm get-parameter --region us-west-2 --name "/teleport/${CLUSTER_NAME}/tokens/node" --query "Parameter.Value" --with-decryption --output text --no-cli-pager
# 992a9725-0a64-428d-8e5e-308e6877743d
```

You can also generate an SSH Service join token using `tctl tokens add --type=node` [as detailed here in our admin guide](/docs/ver/14.x/agents/join-services-to-your-cluster/join-token).
...
To manually join Teleport agents to your cluster, you will need [a join token](/docs/ver/14.x/agents/join-services-to-your-cluster/join-token).
...
to a Teleport Auth Service instance. Follow the instructions in our [Trusted Clusters guide](/docs/ver/14.x/management/admin/trustedclusters).
