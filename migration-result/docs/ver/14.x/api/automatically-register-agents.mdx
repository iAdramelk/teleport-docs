...
clusters](/docs/ver/14.x/kubernetes-access/discovery) in AWS, Azure, and Google Cloud,
as well as [servers](/docs/ver/14.x/server-access/guides/ec2-discovery) on Amazon EC2.
...
Assign the `register-apps-impersonator` role to your Teleport user by running the following
...
<Tabs>
  <Tab title="Local User">
    ...
    Edit `out.yaml`, adding `register-apps-impersonator` to the list of existing roles:
  ...
  <Tab title="GitHub">
    ...
    Edit `github.yaml`, adding `register-apps-impersonator` to the
  ...
  <Tab title="SAML">
    ...
    Edit `saml.yaml`, adding `register-apps-impersonator` to the
  ...
  <Tab title="OIDC">
    ...
    Edit `oidc.yaml`, adding `register-apps-impersonator` to the
...
const (
	...
	proxyAddr      string = ""
	teleportImage  string = "public.ecr.aws/gravitational/teleport-distroless:12.1.5"
...
<Accordion title="Other ways to get resources">
...
<Accordion title="Registering applications dynamically">
...
<Accordion title="Looking up tokens">
...
```code
$ docker image pull public.ecr.aws/gravitational/teleport-distroless:12.1.5
```
