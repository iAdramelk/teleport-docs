...
- [Role requests in commercial Teleport editions](/docs/ver/14.x/access-controls/access-requests/role-requests)
- [Resource requests in commercial Teleport editions](/docs/ver/14.x/access-controls/access-requests/resource-requests)
- [Role requests in Teleport Community Edition](/docs/ver/14.x/access-controls/access-requests/oss-role-requests)
...
Templates](/docs/ver/14.x/access-controls/guides/role-templates#templating-in-access-requests) for more
...
1. Collects all the roles named in `allow.request.search_as_roles`, filtering
   ...
   `deny.request.roles`.
2. Determines which of the remaining roles can access the requested resource.
...
1. Calculate the maximum duration of elevated privileges if the Access Request
   ...
   - The `--max-duration` flag of the [`tsh request
     create`](/docs/ver/14.x/reference/cli/tsh#tsh-request-create) command (if the
...
2. Calculate the session TTL of the certificate the user would receive if
   ...
   - The requested session expiration time, which is the value of the
     ...
     create`](/docs/ver/14.x/reference/cli/tsh#tsh-request-create).
   ...
   - The lowest value of the `options.max_session_ttl` field in the user's
     requested roles.
3. If the maximum duration is greater than zero, set the duration of elevated
...
[`tsh request create`](/docs/ver/14.x/reference/cli/tsh#tsh-request-create) and [`tsh request
review`](/docs/ver/14.x/reference/cli/tsh#tsh-request-review) commands. The format accepted
...
1. Begin with with the base expiration of the Access Request, which a user can
   ...
   create`](/docs/ver/14.x/reference/cli/tsh#tsh-request-create) command. If this is
   unset, the request TTL is one hour.
2. If the user's Teleport session is due to expire before the base expiration
   time, Teleport sets the Access Request expiration to the end of the Teleport
   session.
3. If any of the Teleport roles requested by the Access Request has an
...
4. Return an error if the value of `--request-ttl` is greater than the request
...
2. If one of the user's roles includes the `reason` strategy, the user must
...
2. One user must approve the review, and one user must deny it, as long as the
...
language](/docs/ver/14.x/reference/predicate-language).
...
If you [write your own Access Request plugin](/docs/ver/14.x/api/access-plugin), the
