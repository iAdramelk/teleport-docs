...
<img src="/assets/connect-cluster@2x-7418ac75d8.png" alt="resources tab in Teleport Connect" width="1280" height="720" />
...
<Tabs>
  ...
  <Tab title="Linux">
    ...
    ```code
    $ tar -xf  teleport-connect-14.3.6-linux-*.tar.gz
    ```
  </Tab>
  ...
  <Tab title="Windows">
    ...
    ```code
    $ "Teleport Connect Setup-14.3.6.exe" /S
    ```
  </Tab>
</Tabs>
...
<img src="/assets/connect-ui-overview@2x-2e682a1226.png" alt="user interface of Teleport Connect" width="1280" height="720" />
...
- Teleport Connect v14.1+, on the same major version or one version behind the Teleport Proxy
  Service version. See [version compatibility](/docs/ver/14.x/faq#version-compatibility).
- A local Teleport user: you must authenticate using credentials or passwordless login, and not with
  SSO.
- Permissions to create join tokens (verb `create` for [the `token` resource](/docs/ver/14.x/access-controls/reference#rbac-for-dynamic-teleport-resources)).
...
- Permissions to read and update user objects in the backend (verbs `read` and `update` for [the
  `user` resource](/docs/ver/14.x/access-controls/reference)).
- Permissions to read, update, and create roles in the backend (verbs `read`, `update`, and `create`
  for [the `role` resource](/docs/ver/14.x/access-controls/reference)).

The agent runs as the current system user, not as root. Some features are thus not available, such
as logging in as other system users or [host user creation](/docs/ver/14.x/server-access/guides/host-user-creation).
...
<img src="/assets/connect-my-computer-top-bar@2x-5694a1276d.png" alt="Connect My Computer in the top bar" width="703" height="520" />
...
<img src="/assets/connect-my-computer-setup@2x-15f37c908f.png" alt="Connect My Computer setup" width="703" height="520" />
...
<img src="/assets/connect-my-computer-status@2x-c158e1b98f.png" alt="Connect My Computer status" width="703" height="520" />
...
During the setup, Connect My Computer creates [a join token](/docs/ver/14.x/agents/join-services-to-your-cluster/join-token)
...
Controls Reference Documentation](/docs/ver/14.x/access-controls/reference#rbac-for-dynamic-teleport-resources) for more
...
locks](/docs/ver/14.x/access-controls/guides/locking) on those nodes.
...
When using [trusted clusters](/docs/ver/14.x/management/admin/trustedclusters), the cluster selector allows
...
variable](/docs/ver/14.x/reference/cli/tsh#tsh-environment-variables). For example, logging in to a new cluster
...
certificates](/docs/ver/14.x/management/admin/self-signed-certs) or for demo purposes. We do not recommend
