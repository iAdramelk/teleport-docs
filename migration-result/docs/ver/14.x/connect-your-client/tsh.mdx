...
[`tsh`](/docs/ver/14.x/reference/cli/tsh):
...
to call [`tsh login`](/docs/ver/14.x/reference/cli/tsh#tsh-login) in the beginning.
...
<Tabs>
  <Tab title="Mac">
    ...
    ```bash
    $ curl -O https://cdn.teleport.dev/tsh-14.3.6.pkg
    ```
  </Tab>
  ...
  <Tab title="Mac - Homebrew">
    ...
    <Note>
      ...
      versions. See our [compatibility policy](/docs/ver/14.x/upgrading/overview) for details.
    </Note>
  </Tab>

  <Tab title="Windows - Powershell">
    ```bash
    $ curl.exe -O https://cdn.teleport.dev/teleport-v14.3.6-windows-amd64-bin.zip
  ...
  <Tab title="Linux">
    ...
    For more options (including RPM/DEB packages and downloads for i386/ARM/ARM64) please see our [installation page](/docs/ver/14.x/installation).

    ```bash
    $ curl -O https://cdn.teleport.dev/teleport-v14.3.6-linux-amd64-bin.tar.gz
    $ tar -xzf teleport-v14.3.6-linux-amd64-bin.tar.gz
    $ cd teleport
    $ sudo ./install
...
[CLI Docs - tsh ssh](/docs/ver/14.x/reference/cli/tsh#tsh-ssh)
...
[CLI Docs - tsh login](/docs/ver/14.x/reference/cli/tsh#tsh-login)
...
<Tip>
  ...
  It is recommended to always use [`tsh login`](/docs/ver/14.x/reference/cli/tsh#tsh-login) before using any other `tsh` commands. This allows users to omit `--proxy` flag in subsequent tsh commands. For example `tsh ssh user@host` will work.
</Tip>

A Teleport cluster can be configured for multiple user identity sources. For example, a cluster may have a local user called `admin` while regular users should [authenticate via GitHub](/docs/ver/14.x/access-controls/sso/github-sso). In this case, you have to pass `--auth` flag to `tsh login` to specify which identity storage to use:
...
[CLI Docs - tsh status](/docs/ver/14.x/reference/cli/tsh#tsh-status)
...
[`tsh login`](/docs/ver/14.x/reference/cli/tsh#tsh-login) can also save the user certificate into a
file:
...
[Machine ID](/docs/ver/14.x/machine-id/introduction). This ensures that your automation
...
[CLI Docs - tctl auth sign](/docs/ver/14.x/reference/cli/tctl#tctl-auth-sign)
...
[CLI Docs - tsh ls](/docs/ver/14.x/reference/cli/tsh#tsh-ls)
...
<Tip>
  ...
  Refer them to the [Moderated Sessions guide](/docs/ver/14.x/access-controls/guides/moderated-sessions) for more information on configuring join permissions.
</Tip>
...
[Configure Trusted Clusters](/docs/ver/14.x/management/admin/trustedclusters).
...
[CLI Docs - tsh clusters](/docs/ver/14.x/reference/cli/tsh#tsh-clusters)
...
[Uninstalling Teleport](/docs/ver/14.x/management/admin/uninstall-teleport).

## Further reading

- [CLI Reference](/docs/ver/14.x/reference/cli).
