...
<img src="/assets/team-diagram-5386bab7be.png" alt="Teleport Team architecture diagram" width="1800" height="1292" />
...
<Tip>
  ...
  [Choose an Edition](/docs/ver/16.x/choose-an-edition/introduction).
</Tip>
...
- The `tsh` client tool.
  ...
  For installation instructions, see [Installation Guide](/docs/ver/16.x/installation).
...
2. Start a Docker container on your workstation to prepare a server that you want enroll
...
3. Run the following command to install `curl` and `telnet` from the package management repository:
...
1. Open a browser and go to the address for your Teleport Team cluster. For
   example, if your Teleport Team account is `example`, open
   `https://example.teleport.sh`.

2. Sign in with the account credentials you used to activate your Teleport Team account.

   The first time you sign in, you are prompted to add your first resource:

   <img src="/assets/add-my-first-resource@2x-4342d42558.png" alt="Add your first resource" width="1280" height="720" />

3. Click **Add my first resource**, and you will see the option to enroll
   resources in your Teleport cluster.

4. Type *server* in the search box to filter the list of resources:

   <img src="/assets/choose-resource@2x-84d9c4e112.png" alt="Select resource type" width="1280" height="720" />

5. Click *Ubuntu 14.04+* to register the server in the Docker container.
   ...
   <img src="/assets/paste-script@2x-4408dbfb8f.png" alt="Configure resource" width="1280" height="720" />

6. The Teleport installation script uses `sudo`, which is not installed on your
...
7. Copy the command to run the installation script from the Teleport Web UI and paste it
   ...
   <img src="/assets/successfully-connected@2x-326f829b63.png" alt="Connected successfully" width="1280" height="720" />

   To continue, click **Next**.

8. Confirm that you want to authenticate as the `root` user and click **Next**:

   <img src="/assets/set-up-access@2x-f15c502945.png" alt="Choose your user" width="1280" height="720" />

9. Respond to the MFA prompt again, then click **Test Connection** to verify
   access to the server.

   <img src="/assets/test-connection@2x-aba8982b9a.png" alt="Start session" width="1280" height="720" />
...
2. Execute some commands in your terminal.

3. Enter the `exit` command to end the remote interactive session. Close the
   browser tab.

4. In the last step of **Enroll New Resource**, click **Finish**.

5. Click **Browse existing resources** to see your new server listed in the Teleport Web UI.
...
2. In the sidebar, under **Activity**, click **Session Recordings**.

   You will see the recording for your interactive session from the previous
   step listed.  For example:

   <img
     src="/assets/session-recordings@2x-dfd89fab33.png"
     alt="Session
   recordings"
     width="1280"
     height="720"
...
3. Click **Play** to see a full recording of your session.
...
2. Sign in to your Teleport Team cluster by running the `tsh login` command  with the URL
...
3. List the servers your Teleport user can access.
...
4. Access your server as the `root` user:
...
with Terraform](/docs/ver/16.x/agents/deploy-agents-terraform).
