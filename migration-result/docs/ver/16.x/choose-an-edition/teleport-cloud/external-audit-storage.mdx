...
[Athena](/docs/ver/16.x/reference/backends#athena) backend for audit events and the
[S3](/docs/ver/16.x/reference/backends#s3) backend for session recordings. It is only
...
2. To store and query audit events and session recordings, you need to set up
   ...
   - A transient storage S3 bucket that temporarily holds the results of Athena
     queries.
3. Currently it is a requirement that the AWS infrastructure is hosted in the
   same AWS region as the Teleport Auth Service for your cluster. Historically
   this has always been us-west-2, but new clusters may be deployed to other
...
![Screenshot of External Audit Storage integration tile](/assets/integration-046b475dd3.png)
...
<Accordion title="IAM policy for External Audit Storage">
...
[Event Handler](/docs/ver/16.x/management/export-audit-events) plugin to export
