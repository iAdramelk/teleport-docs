...
<Tip>
  You can use
  [Enhanced Session Recording with BPF](/docs/ver/16.x/server-access/guides/bpf-session-recording)
...
<Tabs>
  <Tab title="Self-Hosted">
    ...
    [Athena](/docs/ver/16.x/reference/backends#athena), [DynamoDB](/docs/ver/16.x/reference/backends#dynamodb) or
    [Firestore](/docs/ver/16.x/reference/backends#firestore) chapters for information on how to
    ...
    [Teleport Configuration Reference](/docs/ver/16.x/reference/config).
...
<Tip>
  ...
  read [Exporting Teleport Audit Events](/docs/ver/16.x/management/export-audit-events).
</Tip>
...
| join\_token.create       | A new join token has been created. Adds the following fields: `{"roles": ["Node", "Db"], "join_method": "token"}`                                                   |
...
<Tabs>
  <Tab title="Self-Hosted">
    Teleport can store the recorded sessions in an [AWS S3 bucket](/docs/ver/16.x/reference/backends#s3)
    ...
    You can replay recorded sessions using the [`tsh play`](/docs/ver/16.x/reference/cli/tsh#tsh-play)
