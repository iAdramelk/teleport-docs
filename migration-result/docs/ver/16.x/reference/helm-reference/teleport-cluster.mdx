...
This includes deploying proxies, auth servers, and [kubernetes-access](/docs/ver/16.x/kubernetes-access/introduction).
See the [Teleport HA Architecture page](/docs/ver/16.x/deploy-a-cluster/high-availability)
for more details.

You can
[browse the source on GitHub](https://github.com/gravitational/teleport/tree/branch/v16/examples/chart/teleport-cluster).
...
| -------------------- | ------------------ | ---------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| `auth_service`       | auth `Deployment`  | Authenticates users and hosts, and issues certificates.                                                                | [Auth documentation](/docs/ver/16.x/architecture/authentication)                 |
| `kubernetes_service` | auth `Deployment`  | Provides secure access to the Kubernetes<br />cluster where the Teleport cluster is hosted.                            | [Kubernetes Access documentation](/docs/ver/16.x/kubernetes-access/introduction) |
| `proxy_service`      | proxy `Deployment` | Runs the externally-facing parts of a Teleport<br />cluster, such as the web UI, SSH proxy and reverse tunnel service. | [Proxy documentation](/docs/ver/16.x/architecture/proxy)                         |
...
<Tip>
  ...
  [`teleport-kube-agent` Helm chart](/docs/ver/16.x/reference/helm-reference/teleport-kube-agent).
...
| ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `standalone`              | Runs by relying only on Kubernetes resources.                                                                                                                                                                                     | [Getting Started - Kubernetes with SSO](/docs/ver/16.x/deploy-a-cluster/helm-deployments/kubernetes-cluster)                 |
| `aws`                     | Leverages AWS managed services to store data.                                                                                                                                                                                     | [Running an HA Teleport cluster using an AWS EKS Cluster](/docs/ver/16.x/deploy-a-cluster/helm-deployments/aws)              |
| `gcp`                     | Leverages GCP managed services to store data.                                                                                                                                                                                     | [Running an HA Teleport cluster using a Google Cloud GKE cluster](/docs/ver/16.x/deploy-a-cluster/helm-deployments/gcp)      |
| `azure`                   | Leverages Azure managed services to store data.                                                                                                                                                                                   | [Running an HA Teleport cluster using a Microsoft Azure AKS cluster](/docs/ver/16.x/deploy-a-cluster/helm-deployments/azure) |
| `scratch` (v12 and above) | Generates empty Teleport configuration. User must pass their own config. This is discouraged, use `standalone` mode with [`auth.teleportConfig`](#authteleportconfig) and [`proxy.teleportConfig`](#proxyteleportconfig) instead. | [Running a Teleport cluster with a custom config](/docs/ver/16.x/deploy-a-cluster/helm-deployments/custom)                   |

<Note>
  `custom` mode has been removed in Teleport version 12. See the [version 12
  migration guide](/docs/ver/16.x/deploy-a-cluster/helm-deployments/migration-v12) for
  more information.
</Note>
...
See the [PROXY Protocol security section](/docs/ver/16.x/management/security/proxy-protocol) for more details.
...
See the [Teleport Configuration Reference](/docs/ver/16.x/reference/config) for the list of supported fields.
...
[The SSO documentation](/docs/ver/16.x/access-controls/sso) explains how to create
...
- [`local`](/docs/ver/16.x/management/admin/users) for local users
- [`passwordless`](/docs/ver/16.x/access-controls/guides/passwordless#optional-enable-passwordless-by-default) to enable by
...
[Disabling local auth is required for FedRAMP / FIPS](/docs/ver/16.x/access-controls/compliance-frameworks/fedramp#configure-the-teleport-auth-service).
...
See [the locking modes documentation](/docs/ver/16.x/access-controls/guides/locking#next-steps-locking-modes) for more
details.
...
See [Second Factor - WebAuthn](/docs/ver/16.x/access-controls/guides/webauthn) for more details.
...
[device verification](https://developers.yubico.com/WebAuthn/WebAuthn_Developer_Guide/Attestation.html).
This field allows you to forbid specific device models and vendors, while allowing all others (provided they clear attestation\_allowed\_cas as well).
...
For possible values, [see the Teleport Configuration Reference](/docs/ver/16.x/reference/config).
...
See our [installation guide](/docs/ver/16.x/installation#docker) for information on
...
By default, Teleport charts used to install a [`podSecurityPolicy`](https://github.com/gravitational/teleport/blob/branch/v16/examples/chart/teleport-cluster/templates/psp.yaml).
...
[PSP removal guide](/docs/ver/16.x/deploy-a-cluster/helm-deployments/migration-kubernetes-1-25-psp).
...
| `chartMode`  | Guide                                                                                                                        |
| ------------ | ---------------------------------------------------------------------------------------------------------------------------- |
| `standalone` | [Getting Started - Kubernetes with SSO](/docs/ver/16.x/deploy-a-cluster/helm-deployments/kubernetes-cluster)                 |
| `aws`        | [Running an HA Teleport cluster using an AWS EKS Cluster](/docs/ver/16.x/deploy-a-cluster/helm-deployments/aws)              |
| `gcp`        | [Running an HA Teleport cluster using a Google Cloud GKE cluster](/docs/ver/16.x/deploy-a-cluster/helm-deployments/gcp)      |
| `azure`      | [Running an HA Teleport cluster using a Microsoft Azure AKS cluster](/docs/ver/16.x/deploy-a-cluster/helm-deployments/azure) |
| `scratch`    | [Running a Teleport cluster with a custom config](/docs/ver/16.x/deploy-a-cluster/helm-deployments/custom)                   |
...
<Accordion title="Read this if using Kubernetes 1.23+ on EKS">
...
`aws` settings are described in the AWS guide: [Running an HA Teleport cluster using an AWS EKS Cluster](/docs/ver/16.x/deploy-a-cluster/helm-deployments/aws)

## `gcp`

`gcp` settings are described in the GCP guide: [Running an HA Teleport cluster using a Google Cloud GKE cluster](/docs/ver/16.x/deploy-a-cluster/helm-deployments/gcp)

## `azure`

`azure` settings are described in the Azure guide: [Running an HA Teleport cluster using a Microsoft Azure AKS cluster](/docs/ver/16.x/deploy-a-cluster/helm-deployments/azure)
...
<Note>
  ...
  and the relevant sections of the [AWS](/docs/ver/16.x/deploy-a-cluster/helm-deployments/aws#step-47-configure-tls-certificates-for-teleport) and [GCP](/docs/ver/16.x/deploy-a-cluster/helm-deployments/gcp#step-36-install-and-configure-cert-manager) guides for more information.
</Note>
...
<Note>
  ...
  of the [AWS](/docs/ver/16.x/deploy-a-cluster/helm-deployments/aws#step-47-configure-tls-certificates-for-teleport) and [GCP](/docs/ver/16.x/deploy-a-cluster/helm-deployments/gcp#step-36-install-and-configure-cert-manager) guides for more information.
</Note>
...
See [https://kubernetes.io/docs/concepts/configuration/secret/#tls-secrets](https://kubernetes.io/docs/concepts/configuration/secret/#tls-secrets) for more information.
...
See the [Teleport config file reference](/docs/ver/16.x/reference/config) for more details on possible values for `extra_fields`.
