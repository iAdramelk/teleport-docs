...
or run a version server already, return to the [Upgrading](/docs/ver/16.x/upgrading)
menu for the appropriate next steps to upgrade Teleport.

The [Automatic Update Architecture](/docs/ver/16.x/architecture/agent-update-management)
...
3. Make the `current` release channel target the version 16.0.0-dev:

   ```bash
   $ echo -n "16.0.0-dev" > current/version
   ```
...
[`teleport_cluster_maintenance_config`](/docs/ver/16.x/reference/terraform-provider#teleport_cluster_maintenance_config)
