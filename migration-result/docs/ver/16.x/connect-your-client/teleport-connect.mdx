...
<img src="/assets/connect-cluster@2x-2c2aaf512a.png" alt="resources tab in Teleport Connect" width="1280" height="720" />
...
<Tabs>
  ...
  <Tab title="Linux">
    ...
    ```code
    $ tar -xf  teleport-connect-16.0.0-dev-linux-*.tar.gz
    ```
  </Tab>
  ...
  <Tab title="Windows">
    ...
    ```code
    $ "Teleport Connect Setup-16.0.0-dev.exe" /S
    ```
  </Tab>
</Tabs>
...
<img src="/assets/connect-ui-overview@2x-cf1e1d01db.png" alt="user interface of Teleport Connect" width="1280" height="720" />
...
2. Look for the SSH server you want to connect to and click the Connect button.
...
When using [trusted clusters](/docs/ver/16.x/management/admin/trustedclusters), the cluster selector allows
...
1. Open a tab with cluster resources by clicking on the plus symbol at the right end of the tab bar.
   You can also press <span>`Ctrl/Cmd + T`</span> to achieve the same result.
2. Look for the cluster you wish to connect to and click the Connect button.
...
2. Look for the database server you wish to connect to and click the Connect button.
3. Select or enter the database user you wish to use and press `Enter`.
...
When it comes to [cloud APIs secured with Application Access](/docs/ver/16.x/application-access/cloud-apis),
...
1. Open a tab with cluster resources by clicking on the plus symbol at the end of the tab bar. You
   can also press <span>`Ctrl/Cmd + T`</span> to achieve the same result.
2. Look for the application you wish to open and click the Launch button (web apps and AWS console)
...
1. Open a tab with cluster resources by clicking on the plus symbol at the end of the tab bar. You
   can also press <span>`Ctrl/Cmd + T`</span> to achieve the same result.
2. Look for the application you wish to connect to.
...
2. Open the profile selector with a shortcut (<span>`Ctrl/Cmd + I`</span>).
3. Select a connection from the connection list at the top to automatically switch to the right profile.
...
- Teleport Connect v14.1+, on the same major version or one version behind the Teleport Proxy
  Service version. See [version compatibility](/docs/ver/16.x/faq#version-compatibility).
- A local Teleport user: you must authenticate using credentials or passwordless login, and not with
  SSO.
- Permissions to create join tokens (verb `create` for [the `token` resource](/docs/ver/16.x/access-controls/reference#rbac-for-dynamic-teleport-resources)).
...
- Permissions to read and update user objects in the backend (verbs `read` and `update` for [the
  `user` resource](/docs/ver/16.x/access-controls/reference#rbac-for-dynamic-teleport-resources)).
- Permissions to read, update, and create roles in the backend (verbs `read`, `update`, and `create`
  for [the `role` resource](/docs/ver/16.x/access-controls/reference#rbac-for-dynamic-teleport-resources)).

The agent runs as the current system user, not as root. Some features are thus not available, such
as logging in as other system users or [host user creation](/docs/ver/16.x/server-access/guides/host-user-creation).
...
<img src="/assets/connect-my-computer-top-bar@2x-37863c2229.png" alt="Connect My Computer in the top bar" width="700" height="584" />
...
<img src="/assets/connect-my-computer-setup@2x-dbdd908932.png" alt="Connect My Computer setup" width="700" height="584" />
...
<img src="/assets/connect-my-computer-status@2x-8fc5e9ad99.png" alt="Connect My Computer status" width="700" height="584" />
...
During the setup, Connect My Computer creates [a join token](/docs/ver/16.x/agents/join-services-to-your-cluster/join-token)
...
Controls Reference Documentation](/docs/ver/16.x/access-controls/reference#rbac-for-dynamic-teleport-resources) for more
...
locks](/docs/ver/16.x/access-controls/guides/locking) on those nodes.
...
variable](/docs/ver/16.x/reference/cli/tsh#tsh-environment-variables). For example, logging in to a new cluster
...
2. Close Teleport Connect.
3. Open Teleport Connect, then log back in to the cluster.

To completely wipe all app state:

1. Close Teleport Connect.
2. Remove the internal `tsh` folder and the `app_state.json` file to log out of all clusters and
...
certificates](/docs/ver/16.x/management/admin/self-signed-certs) or for demo purposes. We do not recommend
