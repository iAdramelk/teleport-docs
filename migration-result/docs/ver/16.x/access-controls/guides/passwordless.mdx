...
- Teleport must be configured for WebAuthn. See the [Second Factor:
  WebAuthn](/docs/ver/16.x/access-controls/guides/webauthn) guide.
...
- A signed and notarized version of `tsh` is required for Touch ID. This precludes
  versions installed from Homebrew. [Download the macOS tsh installer](/docs/ver/16.x/installation#macos).
...
<Accordion title="Registering Touch ID for tsh">
...
[installation page](/docs/ver/16.x/installation).
...
device. Try following the [installation instructions for libfido2](https://github.com/Yubico/libfido2#installation), which may provide you the
necessary udev rules.

### Touch ID not usable

If you are having trouble with Touch ID, make sure that you are using the latest
standalone version of `tsh`. [Download the macOS tsh installer](/docs/ver/16.x/installation#macos).
