...
[integrate with many other providers](/docs/ver/16.x/access-controls/access-requests).

<Warning>
  This guide requires a commercial edition of Teleport. The open source edition of
  Teleport only supports [GitHub](/docs/ver/16.x/access-controls/sso/github-sso) as an
  SSO provider.
</Warning>
...
<Tabs>
  <Tab title="Download">
    ```code
    $ curl -L https://get.gravitational.com/teleport-access-mattermost-v13.3.7-linux-amd64-bin.tar.gz
    $ tar -xzf teleport-access-mattermost-v13.3.7-linux-amd64-bin.tar.gz
  ...
  <Tab title="From Source">
    To install from source you need `git` and `go >= 1.22` installed.
