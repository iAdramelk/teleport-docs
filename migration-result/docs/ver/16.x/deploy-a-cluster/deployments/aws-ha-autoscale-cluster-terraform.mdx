...
```code
$ export TF_VAR_ami_name="teleport-ent-15.0.0-dev-x86_64"
```
...
where `<type>` is either `oss` or `ent` (Enterprise), `<version>` is the version of Teleport, e.g. `15.0.0-dev`,
...
before the `<arch>` component, e.g. `teleport-ent-15.0.0-dev-fips-x86_64`.
...
<Tip>
  ...
  ```code
  $ aws --region us-west-2 ec2 describe-images --owners 146628656107 --filters 'Name=name,Values=teleport-oss-15.0.0-dev-*'
  ```

  Enterprise AMIs<br />

  ```code
  $ aws --region us-west-2 ec2 describe-images --owners 146628656107 --filters 'Name=name,Values=teleport-ent-15.0.0-dev-*'
  ```

  Enterprise FIPS 140-2 AMIs<br />

  ```code
  $ aws --region us-west-2 ec2 describe-images --owners 146628656107 --filters 'Name=name,Values=teleport-ent-15.0.0-dev-*-fips-*'
  ```
</Tip>
...
If set to the string `true`, Teleport will use [TLS routing](/docs/ver/16.x/architecture/tls-routing) to multiplex all traffic over a single port.
...
See the [Teleport authentication reference](/docs/ver/16.x/reference/authentication) for more information.
...
- 2 x `m7g.large` Teleport Proxy Service instances in an ASG, behind a public-facing load balancer - NLB for LetsEncrypt, ALB for ACM. [The desired size of the ASG is configured here](https://github.com/gravitational/teleport/blob/branch/v15/examples/aws/terraform/ha-autoscale-cluster/proxy_asg.tf#L12)
...
More information about how Teleport works with DynamoDB can be found in our [Storage Backends guide](/docs/ver/16.x/reference/backends#dynamodb).
...
ways to integrate Teleport onto your servers. We recommend looking at our [Installation guide](/docs/ver/16.x/installation).

To add new nodes/EC2 servers that you can "SSH into" you'll need to:

- [Install the Teleport binary on the Server](/docs/ver/16.x/installation)
- [Run Teleport - we recommend using systemd](/docs/ver/16.x/management/admin/daemon)
- [Set the correct settings in /etc/teleport.yaml](/docs/ver/16.x/reference/config)
- [Add Nodes to the Teleport
  cluster](/docs/ver/16.x/agents/join-services-to-your-cluster)
...
You can also generate an SSH Service join token using `tctl tokens add --type=node` [as detailed here in our admin guide](/docs/ver/16.x/agents/join-services-to-your-cluster/join-token).
...
To manually join Teleport agents to your cluster, you will need [a join token](/docs/ver/16.x/agents/join-services-to-your-cluster/join-token).
...
to a Teleport Auth Service instance. Follow the instructions in our [Trusted Clusters guide](/docs/ver/16.x/management/admin/trustedclusters).
