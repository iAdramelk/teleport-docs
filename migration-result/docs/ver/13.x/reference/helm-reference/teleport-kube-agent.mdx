...
GitHub](https://github.com/gravitational/teleport/tree/branch/v13/examples/chart/teleport-kube-agent).
...
| --------------------------------------------------------------------- | -------------------------------------- | -------------------------------------------------------------------------------------------- |
| [`kubernetes_service`](/docs/ver/13.x/kubernetes-access/introduction) | `kube`                                 | Uses Teleport to handle authentication<br /> with and proxy access to a Kubernetes cluster   |
| [`application_service`](/docs/ver/13.x/application-access/guides)     | `app`                                  | Uses Teleport to handle authentication<br /> with and proxy access to web-based applications |
| [`database_service`](/docs/ver/13.x/database-access/guides)           | `db`                                   | Uses Teleport to handle authentication<br /> with and proxy access to databases              |
...
- For `iam`, see [Joining Nodes Via AWS IAM Role](/docs/ver/13.x/agents/join-services-to-your-cluster/aws-iam).
- For `ec2`, see [Joining Nodes Via AWS EC2 Identity Document](/docs/ver/13.x/agents/join-services-to-your-cluster/aws-ec2).
...
<Tip>
  You can see a list of all the supported [values which can be used in a Teleport application access configuration here](/docs/ver/13.x/application-access/reference#configuration).
</Tip>
...
<Tip>
  Once `appResources` is set, you can dynamically register application with `tsh` by following [this guide](/docs/ver/13.x/application-access/guides/dynamic-registration).
</Tip>
...
<Note>
  For AWS database auto-discovery to work, your Database Service pods will need to use a role which has appropriate IAM permissions as per the [database documentation](/docs/ver/13.x/database-access/guides/rds#step-36-create-iam-policies-for-teleport).
...
<Note>
  For Azure database auto-discovery to work, your Database Service pods will need to have appropriate IAM permissions as per the [database documentation](/docs/ver/13.x/database-access/guides/azure-postgres-mysql#step-46-configure-iam-permissions-for-teleport).
...
<Tip>
  You can see a list of all the supported [values which can be used in a Teleport database service configuration here](/docs/ver/13.x/database-access/reference/configuration).
</Tip>
...
<Tip>
  Once `databaseResources` is set, you can dynamically register database with `tsh` by following [this guide](/docs/ver/13.x/database-access/guides/dynamic-registration).
</Tip>
...
["Adding Nodes to the Cluster"](/docs/ver/13.x/agents/join-services-to-your-cluster/join-token).
...
<Warning>
  ...
  One option might be to use Teleport's built-in [ACME support](/docs/ver/13.x/reference/helm-reference/teleport-cluster#acme) or enable [cert-manager support](/docs/ver/13.x/reference/helm-reference/teleport-cluster#highavailabilitycertmanager).
</Warning>
...
By default, Teleport charts used to install a [`podSecurityPolicy`](https://github.com/gravitational/teleport/blob/branch/13/examples/chart/teleport-cluster/templates/psp.yaml).
...
<Note>
  ...
  For more information on how to set static/dynamic labels for Teleport services, see [labelling nodes and applications](/docs/ver/13.x/management/admin/labels).
</Note>
