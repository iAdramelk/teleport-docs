...
<Tabs>
  ...
  <Tab title="Cloud-Hosted Teleport">
...
If you don't want to proxy some of this traffic (for example, proxying HTTPS but not SSH), assign `NO_PROXY` to the address of the Teleport Proxy Service endpoint you want to exclude from HTTP\_CONNECT tunneling in `host:port` format:
