...
- The Teleport Desktop Service with at least one remote desktop registered in
  ...
  Started with Desktop Access](/docs/ver/13.x/desktop-access/getting-started) before beginning this
  guide.
...
![The "Share Desktop" button](../../../../../assets/share-97f87d1b5d.png)
...
![Shared directory in File Explorer](../../../../../assets/shared-dir-d29a388afe.png)
...
Assign the `no-sharing` role to your Teleport user by running the following
...
<Tabs>
  <Tab title="Local User">
    ...
    Edit `out.yaml`, adding `no-sharing` to the list of existing roles:
  ...
  <Tab title="GitHub">
    ...
    Edit `github.yaml`, adding `no-sharing` to the
  ...
  <Tab title="SAML">
    ...
    Edit `saml.yaml`, adding `no-sharing` to the
  ...
  <Tab title="OIDC">
    ...
    Edit `oidc.yaml`, adding `no-sharing` to the
...
up [session recording for desktop access](/docs/ver/13.x/desktop-access/reference/sessions).
