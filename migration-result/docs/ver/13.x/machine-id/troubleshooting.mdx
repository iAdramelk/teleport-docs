...
be automatically [locked](/docs/ver/13.x/access-controls/guides/locking).
...
When requesting [Database Access](/docs/ver/13.x/database-access/introduction)
...
Teleport [RBAC permissions](/docs/ver/13.x/access-controls/reference) and are not
allowed to view or list databases by default unless granted permission via one
or more roles.

### Resolution

Per the [Machine ID Database Access Guide](/docs/ver/13.x/machine-id/guides/databases), ensure at
least one role providing database permissions has been granted to the
destination listed in the error.
...
<Note>
  By default, destinations (like `/opt/machine-id`) are granted all roles provided
