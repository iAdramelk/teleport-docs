...
Setting `export TF_VAR_ami_name="gravitational-teleport-ami-ent-13.2.0"`
...
where `<type>` is either `oss` or `ent` (Enterprise) and `version` is the version of Teleport e.g. `13.2.0`.
