...
This guide is designed to accompany our [reference Terraform code](https://github.com/gravitational/teleport/tree/branch/v13/examples/aws/terraform/ha-autoscale-cluster#terraform-based-provisioning-example-amazon-single-ami)
...
```bash
$ git clone https://github.com/gravitational/teleport -b branch/v13
...
`examples/aws/terraform/ha-autoscale-cluster` section of the Teleport repo](https://github.com/gravitational/teleport/blob/branch/v13/examples/aws/terraform/ha-autoscale-cluster/README.md)
...
The AWS region to run in. You should pick from the supported list as detailed in the [README](https://github.com/gravitational/teleport/blob/branch/v13/examples/aws/terraform/ha-autoscale-cluster/README.md). These are regions which support [DynamoDB encryption
...
Setting `export TF_VAR_ami_name="gravitational-teleport-ami-ent-13.4.16"`
...
where `<type>` is either `oss` or `ent` (Enterprise) and `version` is the version of Teleport e.g. `13.4.16`.
...
- 2 x m4.large Teleport **auth** instances in an ASG, behind an internal network load balancer, configured using DynamoDB for
  shared storage. [The desired size of the ASG is configured here](https://github.com/gravitational/teleport/blob/branch/v13/examples/aws/terraform/ha-autoscale-cluster/auth_asg.tf#L11)
- 2 x m4.large Teleport **proxy** instances in an ASG, behind a public-facing load balancer - NLB for LetsEncrypt, ALB for ACM. [The desired size of the ASG is configured here](https://github.com/gravitational/teleport/blob/branch/v13/examples/aws/terraform/ha-autoscale-cluster/proxy_asg.tf#L12)
- 1 x m4.large Teleport **node** instance in an ASG. Server instances in this ASG will join with Teleport SSH Service enabled and connected to the **auth**
  service. [The desired size of the ASG is configured here](https://github.com/gravitational/teleport/blob/branch/v13/examples/aws/terraform/ha-autoscale-cluster/node_asg.tf#L10)
- 1 x m4.large monitoring server in an ASG which hosts the Grafana instance and receives monitoring data from each service in the cluster. [The desired size of the ASG is configured here](https://github.com/gravitational/teleport/blob/branch/v13/examples/aws/terraform/ha-autoscale-cluster/monitor_asg.tf#L12)
...
[The instance types used for each ASG can be configured here](https://github.com/gravitational/teleport/blob/branch/v13/examples/aws/terraform/ha-autoscale-cluster/vars.tf#L23-L45)
...
More information about how Teleport works with DynamoDB can be found in our [Storage Backends guide](/docs/ver/13.x/reference/backends#dynamodb).
...
https\://[route53\_domain](#route53_domain) - this is [https://teleport.example.com](https://teleport.example.com) in our example.
...
ways to integrate Teleport onto your servers. We recommend looking at our [Installation guide](/docs/ver/13.x/installation).

To add new nodes/EC2 servers that you can "SSH into" you'll need to:

- [Install the Teleport binary on the Server](/docs/ver/13.x/installation)
- [Run Teleport - we recommend using systemd](/docs/ver/13.x/management/admin/daemon)
- [Set the correct settings in /etc/teleport.yaml](/docs/ver/13.x/reference/config)
- [Add Nodes to the Teleport
  cluster](/docs/ver/13.x/agents/join-services-to-your-cluster)
...
You can also generate a Node join token using `tctl tokens add --type=node` [as detailed here in our admin guide](/docs/ver/13.x/agents/join-services-to-your-cluster/join-token).
...
for this document) or join your nodes using [Teleport's node tunneling](/docs/ver/13.x/agents/join-services-to-your-cluster/join-token) functionality.
...
to an auth server. Follow the instructions in our [Trusted Clusters guide](/docs/ver/13.x/management/admin/trustedclusters).
