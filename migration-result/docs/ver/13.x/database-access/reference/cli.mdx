...
<Note>
  ...
  user must be be able to impersonate the built-in `Db` role and user. See the [impersonation guide](/docs/ver/13.x/access-controls/guides/impersonation)
...
Displays only the databases a user has access to (see [RBAC](/docs/ver/13.x/database-access/rbac)).
