...
```code
$ export TF_VAR_ami_name="teleport-ent-15.0.2-x86_64"
```
...
where `<type>` is either `oss` or `ent` (Enterprise), `<version>` is the version of Teleport, e.g. `15.0.2`,
...
e.g. `teleport-ent-15.0.2-x86_64-fips`.
...
<Tip>
  ...
  ```code
  $ aws --region us-west-2 ec2 describe-images --owners 146628656107 --filters 'Name=name,Values=teleport-oss-15.0.2-*'
  ```

  Enterprise AMIs<br />

  ```code
  $ aws --region us-west-2 ec2 describe-images --owners 146628656107 --filters 'Name=name,Values=teleport-ent-15.0.2-*'
  ```

  Enterprise FIPS 140-2 AMIs<br />

  ```code
  $ aws --region us-west-2 ec2 describe-images --owners 146628656107 --filters 'Name=name,Values=teleport-ent-15.0.2-*-fips-*'
  ```
</Tip>
