...
This guide is designed to accompany our [reference Terraform code](https://github.com/gravitational/teleport/tree/branch/v15/examples/aws/terraform/ha-autoscale-cluster#terraform-based-provisioning-example-amazon-single-ami)
...
`examples/aws/terraform/ha-autoscale-cluster` section of the Teleport repo](https://github.com/gravitational/teleport/blob/branch/v15/examples/aws/terraform/ha-autoscale-cluster/README.md)
...
The AWS region to run in. You should pick from the supported list as detailed in the [README](https://github.com/gravitational/teleport/blob/branch/v15/examples/aws/terraform/ha-autoscale-cluster/README.md).
...
```code
$ export TF_VAR_ami_name="gravitational-teleport-ami-ent-15.0.0-dev"
```
...
where `<type>` is either `oss` or `ent` (Enterprise) and `version` is the version of Teleport e.g. `15.0.0-dev`.
...
<Tip>
  ...
  ```code
  $ aws --region us-west-2 ec2 describe-images --owners 126027368216 --filters 'Name=name,Values=gravitational-teleport-ami-oss-15.0.0-dev'
  ```

  Enterprise AMIs<br />

  ```code
  $ aws --region us-west-2 ec2 describe-images --owners 126027368216 --filters 'Name=name,Values=gravitational-teleport-ami-ent-15.0.0-dev'
  ```

  List Enterprise FIPS 140-2 AMIs<br />

  ```code
  $ aws --region us-west-2 ec2 describe-images --owners 126027368216 --filters 'Name=name,Values=gravitational-teleport-ami-ent-15.0.0-dev-fips'
  ```
</Tip>
...
If set to the string `true`, Teleport will use [TLS routing](/docs/architecture/tls-routing) to multiplex all traffic over a single port.
...
See the [Teleport authentication reference](/docs/reference/authentication) for more information.
...
- 2 x `m4.large` Teleport Auth Service instances in an ASG, behind an internal network load balancer, configured using DynamoDB for
  shared storage. [The desired size of the ASG is configured here](https://github.com/gravitational/teleport/blob/branch/v15/examples/aws/terraform/ha-autoscale-cluster/auth_asg.tf#L11)
- 2 x `m4.large` Teleport Proxy Service instances in an ASG, behind a public-facing load balancer - NLB for LetsEncrypt, ALB for ACM. [The desired size of the ASG is configured here](https://github.com/gravitational/teleport/blob/branch/v15/examples/aws/terraform/ha-autoscale-cluster/proxy_asg.tf#L12)
- 1 x `m4.large` Teleport SSH Service instance in an ASG. Server instances in this ASG will join with Teleport SSH Service enabled and connected to the **auth**
  service. [The desired size of the ASG is configured here](https://github.com/gravitational/teleport/blob/branch/v15/examples/aws/terraform/ha-autoscale-cluster/node_asg.tf#L10)
- 1 x `m4.large` monitoring server in an ASG which hosts the Grafana instance and receives monitoring data from each service in the cluster. [The desired size of the ASG is configured here](https://github.com/gravitational/teleport/blob/branch/v15/examples/aws/terraform/ha-autoscale-cluster/monitor_asg.tf#L12)
...
[The instance types used for each ASG can be configured here](https://github.com/gravitational/teleport/blob/branch/v15/examples/aws/terraform/ha-autoscale-cluster/vars.tf#L23-L45)
...
More information about how Teleport works with DynamoDB can be found in our [Storage Backends guide](/docs/reference/backends#dynamodb).
...
ways to integrate Teleport onto your servers. We recommend looking at our [Installation guide](/docs/installation).

To add new nodes/EC2 servers that you can "SSH into" you'll need to:

- [Install the Teleport binary on the Server](/docs/installation)
- [Run Teleport - we recommend using systemd](/docs/management/admin/daemon)
- [Set the correct settings in /etc/teleport.yaml](/docs/reference/config)
- [Add Nodes to the Teleport
  cluster](/docs/agents/join-services-to-your-cluster)
...
You can also generate an SSH Service join token using `tctl tokens add --type=node` [as detailed here in our admin guide](/docs/agents/join-services-to-your-cluster/join-token).
...
To manually join Teleport agents to your cluster, you will need [a join token](/docs/agents/join-services-to-your-cluster/join-token).
...
to a Teleport Auth Service instance. Follow the instructions in our [Trusted Clusters guide](/docs/management/admin/trustedclusters).
