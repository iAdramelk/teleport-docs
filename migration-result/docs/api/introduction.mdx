...
programatically interact with the API. [tsh and tctl](/docs/reference/cli) use
the same API.

Here is what you can do with the Go Client:

- Integrate with external tools, e.g., to write an [Access Request
  plugin](/docs/access-controls/access-request-plugins/index). Teleport
...
Guide](/docs/api/getting-started).
...
- [Automatically Register Teleport Agents](/docs/api/automatically-register-agents):
  Write a Teleport API client to reconcile the resources in your Teleport
  cluster with an external service discovery API.
- [Automatically generate Teleport roles](/docs/api/rbac) from an external RBAC
  system, making it easier to get started with Teleport-based RBAC and keep your
  Teleport roles up to date.
- [Write an Access Request Plugin](/docs/api/access-plugin): Follow this guide for a
