...
<Tip>
  Before continuing, please make sure to take a look at the [Cluster State section](/docs/architecture/nodes#cluster-state)
...
address of the load balancer in the [`auth_server`](/docs/reference/config) field when
...
If using the [default configuration](/docs/reference/networking), configure your load
...
`/readyz` [diagnostics endpoint](/docs/management/diagnostics/monitoring) on
machines running Teleport. This endpoint must be enabled by using the
`--diag-addr` flag to teleport start: `teleport start --diag-addr=127.0.0.1:3000` The
...
<Note>
  ...
  [Teleport-event-exporter](/docs/management/export-audit-events/fluentd)
