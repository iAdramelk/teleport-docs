...
Resources](/docs/management/dynamic-resources).
...
Here's the list of resources currently exposed via [`tctl`](/docs/reference/cli/tctl):
...
| connector                                              | Authentication connectors for [Single Sign-On](/docs/access-controls/sso) (SSO) for SAML, OIDC and GitHub.              |
| node                                                   | A registered SSH node. The same record is displayed via `tctl nodes ls`                                                 |
| windows\_desktop                                       | A registered Windows desktop.                                                                                           |
| cluster                                                | A trusted cluster. See [here](/docs/management/admin/trustedclusters) for more details on connecting clusters together. |
| [login\_rule](#login-rules)                            | A Login Rule, see the [Login Rules guide](/docs/access-controls/login-rules) for more info.                             |
| [device](#device)                                      | A Teleport Trusted Device, see the [Device Trust guide](/docs/access-controls/device-trust/guide) for more info.        |
...
Roles not [granting Kubernetes access](/docs/kubernetes-access/introduction) are
...
see [the `kubernetes_resource` documentation](/docs/kubernetes-access/controls#kubernetes_resources)
for a complete list).
...
[Machine ID configuration reference](/docs/machine-id/reference/configuration).
