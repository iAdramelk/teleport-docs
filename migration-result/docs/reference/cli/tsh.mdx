...
| `--auth`               | `local`          | Any defined [authentication connector](/docs/reference/authentication), including `passwordless` and `local` (i.e., no authentication connector) | Specify the type of authentication connector to use.                                                                                                                                      |
...
See [full docs on `tsh puttyconfig` here](/docs/connect-your-client/putty-winscp).
...
<Tip>
  ...
  in their [storage backend](/docs/reference/backends).
...
through Teleport.  See the [OpenSSH Guide](/docs/server-access/guides/openssh) guide
...
Access](/docs/application-access/cloud-apis/azure).
...
- `<cluster>` - the name of the cluster,  see [Trusted Cluster](/docs/management/admin/trustedclusters) for more information.
...
| `--query`  | none             | String                                                       | Query by [predicate language](/docs/reference/predicate-language#resource-filtering) enclosed in single quotes |
