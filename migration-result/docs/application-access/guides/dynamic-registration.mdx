...
instances](/docs/agents/deploy-agents-terraform). And behind the scenes, the
Teleport Discovery Service uses dynamic registration to [register Kubernetes
applications](/docs/application-access/enroll-kubernetes-applications).
...
See the full app resource spec [reference](/docs/application-access/reference#application-resource).
