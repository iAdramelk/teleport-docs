...
please see [helm chart documentation](/docs/reference/helm-reference/teleport-kube-agent#kubernetesdiscovery).
...
For more information you can take a look at [`discovery_service`](/docs/reference/config#discovery-service) and [`app_service`](/docs/reference/config#application-service) configuration references.
...
contain full rewrite configuration in YAML format, same as one would use when configuring an app with dynamic registration syntax (see [documentation](/docs/application-access/guides/connecting-apps#rewrite-redirect)).
