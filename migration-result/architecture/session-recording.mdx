...
[trusted clusters](/management/admin/trustedclusters) with their own
...
specified by the `audit_sessions_uri` field in the `teleport.yaml` [configuration file](/reference/config).
...
the [`tsh play`](/reference/cli/tsh#tsh-play) command. Desktop session
...
- [SSH session recording](/architecture/nodes#ssh-session-recording)
- [SSH recording modes](/reference/audit#modes)
