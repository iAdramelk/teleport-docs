...
that this cluster trusts. In this case, Teleport activates [trusted cluster mapping logic](/architecture/trustedclusters#role-mapping).
...
[`tctl users add`](/reference/cli) or API call.
...
[trusted cluster mapping logic](/architecture/trustedclusters#role-mapping) to support this use case.
...
<Tip>
  You can use `where` fields in all resource rules. Check out [the full role reference](/access-controls/reference) contains full role spec for details.
</Tip>
...
- [Access Control Reference](/access-controls/reference).
- [Teleport Predicate Language](/reference/predicate-language).
- [Access Requests Guides](/access-controls/access-requests)
- [Architecture Overview](/core-concepts)
- [Teleport Auth](/architecture/authentication)
