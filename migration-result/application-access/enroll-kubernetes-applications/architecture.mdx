...
are considered of an "app" type, but it can be changed for a service by Kubernetes annotation [`teleport.dev/discovery-type`](/application-access/enroll-kubernetes-applications/reference#teleportdevdiscovery-type).
...
[`teleport.dev/name`](/application-access/enroll-kubernetes-applications/reference#teleportdevname).
...
Otherwise, this port is ignored. But if annotation [`teleport.dev/protocol`](/application-access/enroll-kubernetes-applications/reference#teleportdevprotocol) is used on the service and its value is
