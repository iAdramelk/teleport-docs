...
- A running Teleport cluster in Kubernetes. For details on how to set this up,
  see [Guides for running Teleport using Helm](/deploy-a-cluster/helm-deployments).
...
- The Auth Service ServiceAccount must be granted the `system:auth-delegator`
  ...
  chart](/reference/helm-reference/teleport-cluster) version 12 or
...
- The possible values for `teleport-kube-agent` chart are documented
  [in its reference](/reference/helm-reference/teleport-kube-agent).
- See [Application Access Guides](/application-access/guides)
- See [Database Access Guides](/database-access/guides)
