...
Reference](/reference/config#enabling-teleport-services) for which
...
agents](/agents/deploy-agents-terraform).
...
Cluster](/agents/join-services-to-your-cluster) guides.
...
e.g., [using a Terraform module](/agents/deploy-agents-terraform). To modify the
...
- **Dynamic**: Apply a [configuration
  resource](/management/dynamic-resources) that configures a resource to
  proxy.
...
[Read our guide](/agents/deploy-agents-terraform) to deploying a pool of agents
