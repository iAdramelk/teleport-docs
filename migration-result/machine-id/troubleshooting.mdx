...
be automatically [locked](/access-controls/guides/locking).
...
When requesting [Database Access](/database-access/introduction)
...
Teleport [RBAC permissions](/access-controls/reference) and are not
allowed to view or list databases by default unless granted permission via one
or more roles.

### Resolution

Per the [Machine ID Database Access Guide](/machine-id/access-guides/databases), ensure at
