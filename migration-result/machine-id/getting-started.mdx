...
ID](/machine-id/deployment) guides.
...
Machine ID can join with a token or the [IAM Method](/agents/join-services-to-your-cluster/aws-iam) on AWS.
...
- Read the [architecture overview](/machine-id/architecture) to learn about how
  Machine ID works in more detail.
- Check out the [deployment guides](/machine-id/deployment) to learn about
  configuring `tbot` in a production-ready way for your platform.
- Check out the [access guides](/machine-id/access-guides) to learn about configuring
