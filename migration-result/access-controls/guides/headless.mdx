...
- A v12.2+ Teleport cluster with WebAuthn configured.
  See the [Second Factor: WebAuthn](/access-controls/guides/webauthn) guide.
- WebAuthn hardware device, such as YubiKey.
- Machines for Headless WebAuthn activities have [Linux](/installation#linux), [macOS](/installation#macos) or [Windows](/installation#windows-tsh-client-only) `tsh` binary v12.2+ installed.
