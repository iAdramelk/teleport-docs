...
- Teleport must be configured for WebAuthn. See the [Second Factor:
  WebAuthn](/access-controls/guides/webauthn) guide.
...
- A signed and notarized version of `tsh` is required for Touch ID. This precludes
  versions installed from Homebrew. [Download the macOS tsh installer](/installation#macos).
...
standalone version of `tsh`. [Download the macOS tsh installer](/installation#macos).
