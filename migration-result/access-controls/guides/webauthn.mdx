...
<Tip>
  ...
  *registered* security key" step. This may be undesirable at times, so you may
  ...
  Reference](/reference/cli/tsh#tsh-global-flags) page.
</Tip>
...
<Note>
  WebAuthn for logging in to Teleport is only required for [local users](/reference/authentication#local-no-authentication-connector).
...
- [Passwordless](/access-controls/guides/passwordless)
- [Set up per-session MFA checks](/access-controls/guides/per-session-mfa)
