...
- a Teleport [RBAC](/access-controls/reference) role by the role's name
- a Teleport [trusted device](/access-controls/device-trust/enforcing-device-trust#locking-a-device) by the device ID
...
- an [Access Request](/access-controls/access-requests) by UUID
...
<Accordion title="Under the hood: Lock resource and expiration">
  ...
  as per usual. See the [Admin Guide](/reference/resources) for more
  details.
</Accordion>
